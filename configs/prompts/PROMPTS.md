...
<details open>
<summary>完整文本，27,551 字符</summary>
...
- 交互式运行中研究者在线：走到真正的方向分叉（首次完整验证前的路线选择、探针成本超预算需要取舍、研究者指令之间或指令与验证结果冲突）时，用 `ask_user` 附上你的分析与建议征询一次，而不是自行猜测研究者意图；其余情况自主决策。收到 `unattended` 说明无人值守，不要再问。
...
<details>
<summary>完整文本，18,606 字符</summary>
