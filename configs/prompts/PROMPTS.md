...
<details open>
<summary>完整文本，16,751 字符</summary>
...
- `candidate.py`：推荐用于候选筛选，可读取 `/mnt/snapshot`，可在循环中调用 `from mq_tools import nl` 后使用 `nl(code, prompt="...")`。
...
- NL 工具：`mq_tools.nl(ts_code, prompt=...)` 是决策阶段工具，只能在 `main.py` / `candidate.py` / helper 生成 `trade_intents` 前调用。它在宿主侧启动一个可调用 `text_retrieve` 的 Sub Agent，返回 result dict（含 `status`、`content`、`tool_calls`、`evidence`、`error`）；内容不限定格式，需要数值或标签时须在决策代码中自行解析并写入 `trade_intents.params`。逐分钟交易函数不提供 `ctx.nl`、`ctx.model_dir` 或 `ctx.workspace_dir`。
...
```json
{
  ...
  "identity": {
    ...
    "generated_at": "2026-06-26T13:59:05.067253+00:00",
    "phase": "exploration",
    "run_id": "run_sample",
    "session_kind": "fold"
  },
...
| `backtest` | （无） | 验证回测；Environment 只运行当前 `output/main.py`，决策代码调用 `mq_tools.nl()` 才会触发 NL 工具 |
...
<details>
<summary>完整文本，15,335 字符</summary>
...
- 默认先使用直连网络。只有直连失败、明显卡顿，或任务明确需要代理时，才在单条命令前临时把 runtime env 中列出的 `MQ_PROXY_*` 别名映射为标准代理变量；如果 runtime env 没有列出代理别名，不要自行设置代理。
...
```json
{
  ...
  "identity": {
    ...
    "generated_at": "2026-06-26T13:59:05.067685+00:00",
    "run_id": "run_sample",
...
<details>
<summary>完整文本，15,547 字符</summary>
