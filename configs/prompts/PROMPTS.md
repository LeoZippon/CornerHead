...
<details open>
<summary>完整文本，26,145 字符</summary>
...
- NL 与做空：`ctx.nl(ts_code, prompt=...)` 用于单股文本分析，`ctx.nl(prompt=...)` 用于事件/主题/行业/宏观文本检索；文本按数据节点 PIT 滚动且受配额限制，证据必须降权使用。默认做空券源由成交当日 `margin_secs` 校验，当日集合缺失时按数据缺口拒单（`margin_secs_data_missing`），不可融券会拒单。
...
<details>
<summary>完整文本，17,714 字符</summary>
