...
_FIELDS: list[dict[str, object]] = [
    ...
    {"key": "include_intraday", "group": "数据域", "label": "分钟线域", "type": "bool",
     "help": "可选历史分钟行情（决策快照窗口 + 回放市场事件）；关闭后不加载分钟数据，但 Environment 仍按固定交易分钟时钟推进，仅在 09:30/15:00 使用日线开收盘事件撮合。"},
    ...
    {"key": "intraday_decision_minutes", "group": "回放执行", "label": "日内决策粒度（分钟）", "type": "int",
     "help": "固定交易分钟时钟上 main(ctx) 的调用间距（默认 1 = 每分钟）。订单激活、PIT/状态时钟仍逐分钟推进，只有存在行情事件时才撮合；09:15/09:25/14:57 与盘外 tick 不受影响。调大可缩短回测耗时，但降低日内反应粒度。"},
    {"key": "execution_lag_bars", "group": "回放执行", "label": "执行滞后（交易分钟）", "type": "int", "advanced": True,
     "help": "决策到订单进入 Broker 的固定交易分钟滞后；激活分钟没有行情时订单继续挂起，等待下一市场事件撮合。"},
