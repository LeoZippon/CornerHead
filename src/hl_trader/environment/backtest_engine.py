...
from hl_trader.environment.executor import ExecResult
...
_FORBIDDEN_PATHS = _path_roots("MQ_FORBIDDEN_PATHS")
_WRITE_FORBIDDEN_PATHS = _path_roots("MQ_WRITE_FORBIDDEN_PATHS")
_DISABLE_LINKS = os.environ.get("MQ_DISABLE_LINKS", "") == "1"
...
def _nl(ts_code="", prompt="", *, timeout_seconds=None, content_only=False, **kwargs):
    request_path = os.environ.get("MQ_NL_REQUESTS_PATH", "")
    response_path = os.environ.get("MQ_NL_RESPONSES_PATH", "")
    ...
    timeout = float(timeout_seconds or os.environ.get("MQ_NL_TOOL_TIMEOUT_SECONDS", "300"))
...
tools_module = types.ModuleType("mq_tools")
tools_module.nl = _nl
sys.modules["mq_tools"] = tools_module
...
context = {
    "snapshot_dir": os.environ.get("MQ_SNAPSHOT_DIR", "/mnt/snapshot"),
    "agent_output_dir": os.environ.get("MQ_AGENT_OUTPUT_DIR", "/mnt/agent/output"),
    "model_dir": os.environ.get("MQ_MODEL_DIR", "/mnt/agent/models"),
    "decision_time": os.environ.get("MQ_DECISION_TIME", ""),
    "replay_granularity": os.environ.get("MQ_REPLAY_GRANULARITY", "minute"),
    "nl": _nl,
}
...
def run_strategy_program(
    ...
    *,
    ...
    ``output/main.py``. The optional ``mq_tools.nl`` helper is
    ...
    try:
        ...
        env = {
            "MQ_SNAPSHOT_DIR": executor.map_path(paths.snapshot),
            "MQ_AGENT_OUTPUT_DIR": executor.map_path(paths.agent_output),
            "MQ_MODEL_DIR": executor.map_path(paths.model_artifacts),
            "MQ_DECISION_TIME": decision_time,
            "MQ_REPLAY_GRANULARITY": replay_granularity,
            "MQ_NL_REQUESTS_PATH": executor.map_path(requests_host),
            "MQ_NL_RESPONSES_PATH": executor.map_path(responses_host),
            "MQ_NL_TOOL_TIMEOUT_SECONDS": str(timeout_seconds),
            "MQ_FORBIDDEN_PATHS": _executor_pathsep_join(
                executor,
...
class StrategyPolicyRunner:
    ...
    def __enter__(self) -> "StrategyPolicyRunner":
        try:
            ...
            env = {
                "MQ_SNAPSHOT_DIR": self.executor.map_path(self.paths.snapshot),
                "MQ_AGENT_OUTPUT_DIR": self.executor.map_path(self.paths.agent_output),
                "MQ_DECISION_TIME": self.decision_time,
                "MQ_REPLAY_GRANULARITY": self.replay_granularity,
                "MQ_FORBIDDEN_PATHS": _executor_pathsep_join(
                    self.executor,
                    [
                        self.paths.train,
                ...
                "MQ_WRITE_FORBIDDEN_PATHS": self.executor.map_path(self.paths.agent_output),
                "MQ_DISABLE_LINKS": "1",
            }
