...
Every successful formal validation backtest snapshots the current factor/ and
nl_prior/ into ``/mnt/artifacts/steps/<node_id>/`` and appends a node (with a
parent pointer) to ``steps/tree.json``. The tree accumulates across Folds of
one Experiment: the Pipeline hands it to the next Fold's sandbox and positions
``current_node_id`` at the parent artifact, so the Agent can read where it
stands in the search history. The feature is toggleable for ablations
(``step_tree_enabled``).
"""
...
from hl_trader.environment.artifacts import copy_artifact
...
class StepTree:
    ...
    def position_for_hash(self, artifact_hash: str) -> str | None:
        """Most recent node carrying the given artifact hash (parent lookup)."""
        for node in reversed(self.data["nodes"]):
            if node.get("artifact_hash") == artifact_hash:
    ...
    def render_ascii(self) -> str:
        ...
        def walk(parent_id: str | None, depth: int) -> None:
            for node in children.get(parent_id, []):
                ...
                lines.append(f"{'  ' * depth}- {node['node_id']}{ret_text}{marker}")
