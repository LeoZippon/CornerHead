...
from autotrade.environment.sandbox import DockerSandbox, LocalSandbox, SandboxSpec, link_copytree, probe_image_runtime, resolve_image_identity
...
def main() -> int:
    ...
    # Pin to the immutable image id so probe, container, and records agree.
    spec = __import__("dataclasses").replace(spec, image=resolve_image_identity(spec.image)[0])
