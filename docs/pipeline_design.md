...
- [2. Fold 时间定义](#2-fold-时间定义)
  - [2.1 可配置周期滚动](#21-可配置周期滚动)
  - [2.2 泄漏边界](#22-泄漏边界)
  - [2.3 运行约束](#23-运行约束)
- [3. Step 输入与执行](#3-step-输入与执行)
  - [3.1 Step 输入](#31-step-输入)
  - [3.2 执行流程](#32-执行流程)
  - [3.3 Step 摘要](#33-step-摘要)
- [4. Fold 验收、冻结和测试](#4-fold-验收冻结和测试)
  - [4.1 冻结条件](#41-冻结条件)
  - [4.2 完整验证](#42-完整验证)
  - [4.3 收敛和早停](#43-收敛和早停)
  - [4.4 冻结测试](#44-冻结测试)
- [5. 策略产物冻结](#5-策略产物冻结)
  - [5.1 冻结目录](#51-冻结目录)
  - [5.2 Manifest 字段](#52-manifest-字段)
  - [5.3 Hash 边界](#53-hash-边界)
- [6. Epoch 与元学习](#6-epoch-与元学习)
  - [6.1 元学习输入输出](#61-元学习输入输出)
  - [6.2 元学习边界](#62-元学习边界)
  - [6.3 多 Epoch](#63-多-epoch)
- [7. Development 与 Held-out](#7-development-与-held-out)
  - [7.1 Development](#71-development)
  - [7.2 Held-out](#72-held-out)
- [8. 账本、路径和报告](#8-账本路径和报告)
  - [8.1 实验目录](#81-实验目录)
  - [8.2 路径角色](#82-路径角色)
  - [8.3 主账本](#83-主账本)
  - [8.4 报告](#84-报告)
- [9. 失败条件](#9-失败条件)
  - [9.1 Fail-fast 列表](#91-fail-fast-列表)
- [10. 验收清单](#10-验收清单)
  - [10.1 清单](#101-清单)
...
| 验证决策时点 | 2021 年 10 月第一个交易日 09:25 北京时间 |
| 验证可见数据 | Environment 配置窗口内、截至验证决策时点已可见的数据 |
| 测试决策时点 | 2022Q1 第一个交易日 09:25 北京时间 |
...
- Agent 在验证决策时点只能看到验证区间开始前已可见的数据。
...
- 上一 Fold 的 Agent 对话历史、Shell/LLM/Tool 调用日志、`results/test_*`、测试收益和测试 conversation log 不能进入下一 Fold prompt 或策略产物。
- 如果某个历史区间在后续 Fold 中成为验证区间，当前 Fold 必须重新调用 `backtest_tool` 生成自己的 `results/valid_*`。
...
每个 Fold 默认 60 分钟，Step 共享同一个 Fold deadline。距离 deadline 进入收尾窗口时，Runner/Proxy 最多触发一次固定收尾提示。主对话 `max_llm_calls` 只限制 Agent 行动轮次；context compact 使用独立低成本模型、独立调用上限和独立 trace 事件，但仍受同一个 Fold deadline 约束。超过 `fold_deadline_at` 后，Pipeline 必须截断当前 Fold，停止新的 Shell、服务调用和 LLM 调用。

## 3. Step 输入与执行

Step 是验证期的一次策略修改和验证。每个 Fold 的 Step 数有固定上限，实际执行仍受 Fold deadline 和 `finish_fold_tool` 约束。
...
| `valid_decision_input` | `backtest_tool` 验证模式绑定到 `/mnt/snapshot` 的决策输入视图；`train_snapshot` 是它的 Agent-visible alias，manifest 记录相同 snapshot hash |
...
| `execution_policy` | 允许调用的 Shell/Tool、主对话上限、compact 上限和超时 |
...
7. Agent 主动调用 `modification_check_tool`；`backtest_tool` 在正式执行前也会复核最近检查结果和当前策略/model hash。
8. 修改检查通过后，Agent 调用 `backtest(mode="valid")`；若检查缺失或过期，`backtest_tool` 自动补跑。
9. `backtest_tool` 执行 `output/main.py`，写入 `results/valid_<idx>/`。
10. Pipeline 把完成验证回测的 Step 轻量摘要追加到当前 Fold 记录的 `steps[]`。
11. Agent 根据结果继续下一 Step，或调用无参数 `finish_fold_tool`。
...
实验 CLI 可用 `--min-return`、`--min-sharpe`、`--max-drawdown` 和 `--allow-incomplete-validation` 覆盖这些阈值；生产实验应保留严格规则，流程烟测可显式放宽。
...
`finish_fold_tool` 成功后，Runner 锁定 `output/` 和 `models/` 写入，并停止本 Fold 的 Agent 调用。Pipeline 选择最近一次通过完整验证且当前策略/model hash 未变的 Step。若没有可接受 Step：
...
- Agent 停止，`sandbox_shell_tool` 不再可用。
...
- 第一个 Fold 的可见数据：`valid_decision_input` 绑定为 `/mnt/snapshot`，并作为 `/mnt/snapshots/train` 的只读 alias；验证回放区间安装为 `/mnt/snapshots/valid`；test 和 held-out 不进入元学习可见输入。
...
- 可选的小幅正则化策略产物和模型参数产物。
- 可选 `workspace/sandbox_environment.json`：声明后续 Fold 需要的稳定 Python/npm/apt 依赖。Pipeline 以当前普通 Fold Sandbox 镜像为 base，生成派生 Dockerfile 并执行 `docker build`；构建成功后，后续普通 Fold 和 held-out 使用新 image。构建失败时实验显式失败，不静默回退旧环境。
- meta-learning run manifest 和 canonical `artifacts/run_<id>/agent_trace.jsonl`。

`experiments/<id>/meta_learning/<epoch>/` 只保留便于人工查看的 `taste.md`。元学习 trace 不在该目录重复保存；账本记录 `agent_trace_ref` 指向 canonical run trace，避免 token/API 成本统计和审计时重复计数。Pipeline 会把 `taste.md` 内容作为 `taste_prompt` 注入本 Epoch 每个 Fold 的 run manifest 和 Agent Prompt；后续 Fold 不重新生成 Taste，只继承同一 Epoch 的 Taste。
