...
## 2026-06-26 09:15 pre-open info tick + WS2 rolling daily as-of view (feat/rolling-asof)

- 09:15 info tick (commit `74e75eb`): the pre-open auction now emits two ticks per replay day — a 09:15 info tick (auction not yet matched, `ctx.price` is None, ~10-minute decision window) and the 09:25 priced tick (matched open). `_auction_rows` stamps an `auction_open` fill reference on both, and `_fill_price` uses it so orders placed at either tick fill at the day open under the daily limit rules (`price_label="auction"`); `_market_state` does not expose `auction_open`, so 09:15 keeps `ctx.price=None`. Config `auction_preopen_time` (default "09:15", None disables). Known footgun: at the price-less info tick the optimistic `_Broker` cannot resolve shares, so position-conditional multi-orders don't self-dedup there (documented; the intra-minute-view test now acts at a priced tick). Test: a 09:15 blind entry fills at the open.
- WS2 rolling daily as-of: `_write_asof_daily` builds, per replay day D, `daily.parquet` = frozen-snapshot daily history ∪ replay_daily rows with `trade_date < D` (visible after the prior close; same-day/future excluded), de-duped on (trade_date, ts_code); universe copied from the snapshot. `_resolve_asof_dir` writes it to `workspace/.asof/<D>` (strategy-readable, host-written) and maps the container path; `run_main_ctx_replay(asof_view_enabled, snapshot_dir=...)` sets `state["asof_dir"]` per day and the driver exposes `ctx.asof_dir` (defaulting to `ctx.snapshot_dir` when WS2 is off). Other domains (events/text/fundamentals/intraday) stay on the frozen `ctx.snapshot_dir` in v1. `collect_artifacts` ignores `.asof`. Config `rolling_asof_enabled` (default True) flows ExperimentConfig → run manifest → backtest_tool → engine. Docs: template `candidate.py` reads `ctx.asof_dir`; prompt + agent_design §5.3 + environment_design §7.2 updated; PROMPTS.md regenerated.
- Leakage test `test_rolling_asof_view_excludes_today_and_future`: a main(ctx) at 20220105 09:15 asserts the as-of daily holds 20211230 (history) + 20220104 (prior replay day) but not 20220105 (today) or 20220331 (future), then buys; a leak raises and fails the backtest.
- Validation: `compileall` OK; full suite 288 passing (skipped=2). Remaining: symmetric closing-auction tick (14:57 decision → 15:00 close fill); rolling events/text/fundamentals; M2 NL O(N^2) file re-read (offset). An independent audit of WS2 is being run next given its leakage sensitivity.

## 2026-06-26 Next-bar execution + screen/order split + NL byte-offset reads + audit cleanup (feat/rolling-asof)

- Audit dead-code cleanup (commit `168908d`): an independent repo-wide audit (conciseness/completeness/correctness) drove removal of proven-unreferenced code — the 5 `SimBroker.buy/sell/short/cover/close` convenience wrappers (the engine drives `execute()`), `gpu.select_gpu`, `folds.previous_quarter/next_quarter/quarter_range`, `audit.json_group_counts` — plus unused imports in `backtest_engine`/`experiment`/`common`, and two stale docstrings (`backtest_engine` module, `broker.trades_for`). Correctness fix: gate `price_label="auction"` on `auction_enabled` so a real bar at 09:15/09:25 is not mislabeled. The audit's claim that `common.has_pagination_probe` was unused was wrong (it is a re-export consumed 4× by `audit.py`) — verified and kept. Deferred items A1/C2/C3 left for later.
- Next-bar execution (rewrote `run_main_ctx_replay`): a new `_day_tick_plan(minute_rows, auction_enabled, preopen, decision)` returns ordered `_Tick(minute_key, group, fill_key, is_real, is_auction)` records. A decision on real bar *i* fills at bar *i+1*; the last bar's orders are dropped. With auction on, a 09:15 info tick (empty group → `ctx.price` None) fills at the first real bar (09:30 opening auction); a 09:25 tick (each code's matched open, vol/amount cleared) fills at the second real bar (09:31, first continuous). The day loop keeps `pending: fill_key -> [(action, label)]`, executes due fills at each real bar's OPEN before calling `main` there, and labels pre-open-decided fills `price_label="auction"`. Fill price is the fill bar's `open` via the new `_open_price`; removed `_auction_rows`, the `auction_open` mechanism, and `_fill_price` (next-bar fills the open naturally) and the dead `decision_time_iso` parameter (+ its caller in tools/backtest.py). Contract consequence: `ctx.positions` reflects FILLED positions only, so the Agent must place each entry once on a decision tick and track in-flight intent in `ctx.state_dir`.
- Template recommended cadence: `main` calls `screen_targets(ctx)` at 09:15 (screen + `nl()`, write `state_dir/targets.json`) and `open_targets(ctx)` at 09:25 (read targets, submit; fills at 09:31). `candidate.py` split into the two stages; `trading.py` docstring notes next-bar fills + idempotent management; README + main.py docstrings rewritten.
- M2 NL byte-offset incremental reads: `_serve_nl_requests(..., offset)` (host) and the driver's `_read_responses` both seek past a stored offset and consume only whole lines (`rpartition(b"\n")`), leaving a partial trailing line for the next pass — eliminating the O(N^2) full re-read with no data loss. `MainPolicyRunner` holds `_nl_offset`; the driver keeps `_RESP_STATE = {offset, responses}` accumulating responses. Exercised end-to-end by the NL flow test.
- Tests updated for next-bar semantics: `test_main_ctx_replay` adds an `_auction_minutes()` fixture (09:30 auction print + 09:31 first-continuous bar) so 09:15→09:30 and 09:25→09:31 fills are asserted at the next bar's open; `test_broker_engine` ReplayIntegrationTest strategies moved to the decide-once pattern (order at 09:25 / a single bar) and assert next-bar fills; `test_tools_flow` daily strategies guarded to a single 09:25 tick and minute fill labels/prices updated to the next bar (09:31→14:57, 09:31→09:32).
- Validation: full `unittest discover` 289 passing (skipped=2); `git diff --check` clean. Docs: `environment_design.md` §7.2, `agent_design.md` §5.2/§5.3, and the Fold system prompt (`agent/prompts.py`) describe next-bar execution + the two pre-open ticks + decide-once/state_dir; `PROMPTS.md` regenerated.

## 2026-06-26 QMT-faithful execution: execution_lag_bars + ctx.broker.pending (feat/rolling-asof)

- Constraint: the live QMT deployment has no broker-side conditional/stop orders, so the backtest must not fill engine-side limit/stop orders (it would learn an edge from fills QMT can't reproduce). The execution model is therefore market-orders-only, mirroring a live per-minute polling controller: `main(ctx)` evaluates conditions on closed bars and submits market orders; the conditional logic lives in Agent code, not the broker.
- `execution_lag_bars` (default 2): the gap from an order's decision bar to its fill bar. `_day_tick_plan(... , execution_lag_bars)` maps a continuous real bar at index *i* to `real_keys[i + lag]` (or None near the close); the two pre-open auction ticks keep their fixed targets (09:15 -> first real bar = the 09:30 opening auction; 09:25 -> second real bar = 09:31) and are independent of the lag. lag=1 is the immediate next bar; lag=2 models one bar of compute/submit latency then a fill on the following bar (a 09:35 signal fills at 09:37's open). Market fills still take the fill bar's open. A decision with no bar `lag` ahead records a `main_actions_unfilled` event. Config flows ExperimentConfig.execution_lag_bars -> Fold run manifest -> backtest_tool -> `run_main_ctx_replay`.
- `ctx.broker.pending(ts_code)` (working-order query, parity with the live order query): each tick the host aggregates the still-queued `pending` orders by ts_code (`_working_orders`) into `state["pending"]`; the driver `_Broker` stores it as `self._working` and `pending(code)` returns that code's working orders plus any submitted in the current tick. The Agent's `if not ctx.broker.pending(code) and ctx.broker.position(code) == 0:` de-dups in-flight orders identically in backtest and live, retiring the `ctx.state_dir` intent-tracking added for next-bar (state_dir now only holds the day's screening targets). Test `test_pending_query_dedups_in_flight_orders`: with a dense minute day, a continuous strategy that re-evaluates every tick submits exactly one buy because the 09:30 order is visible via `pending()` at 09:31 before it fills at 09:32.
- Template + docs: `candidate.open_targets` and `trading.py` gate on `ctx.broker.pending`; `main.py`/README/environment_design §7.2/agent_design §5.2-5.3/prompt rewritten to "market orders + execution_lag_bars + pending"; PROMPTS.md regenerated.
- Test adaptations for lag=2: auction-tick entries are lag-exempt and unchanged; continuous-decision tests were given denser minute fixtures (added 09:32/09:33 bars so a 09:31 decision fills 2 bars later at a clear intraday bar) or moved to the 09:25 auction tick (mid-replay entry, intra-tick dedup); the open-bar-leak test passes `execution_lag_bars=1` since it exercises bar synthesis, not timing.
- Validation: full `unittest discover` 290 passing (skipped=2); `git diff --check` clean.

## 2026-06-27 Limit orders (FIX_PRICE) + per-day order book, aligned to xtquant (feat/rolling-asof)

- xtquant verification: the official miniQMT trader doc (https://dict.thinktrader.net/nativeApi/xttrader.html) confirms `order_stock(account, stock_code, order_type, order_volume, price_type, price, strategy_name, order_remark)`; `price_type=FIX_PRICE` is a resting exchange limit order (fills when the market touches the price, stays pending otherwise, cancelable via `cancel_order_stock`, valid for the day). order_type constants: STOCK_BUY/STOCK_SELL, CREDIT_FIN_BUY (margin buy), CREDIT_SLO_SELL (short), CREDIT_BUY_SECU_REPAY (cover); market price_types MARKET_PEER_PRICE_FIRST etc.; queries query_stock_orders(cancelable_only), query_stock_trades, query_stock_positions. So a plain limit order IS QMT-supported (distinct from broker-side conditional/stop 条件单); my earlier "no limit orders" was wrong and is corrected.
- Engine refactor: the single-bar `pending` dict became a per-day order book. `_Order` (name, ts_code, amount/weight, limit_price, activate_index, expire_index, is_auction, status) + `_make_order` (action -> order; limit makes it FIX_PRICE resting valid_bars bars, default 1; market valid 1 bar; close always market) + `_fill_or_rest` (each real bar: promote incoming->working at activate_index, fill working orders against the bar, cancel expired) + `_fill_price_for` (market=open; limit buy/cover fills at min(open,P) when open<=P or low<=P, sell/short at max(open,P) when open>=P or high>=P). `_day_tick_plan` now tags each tick with `activate_index` (decision index + execution_lag_bars; auction ticks fixed at 0 and min(1,n-1)). Unfilled-at-close limit orders and last-bar decisions record `order_cancelled`/`main_actions_unfilled`.
- Maker fills carry no slippage: `SimBroker.execute(..., apply_slippage=True)` threads to `_open`/`_reduce`/`_fill_long_open`/`_fill_short_open`; the engine passes `apply_slippage=order.limit_price is None`, so a limit fill is exactly the limit price (commission still applies; its cost is fill risk, modelled by the touch-or-cancel rule). Market fills keep slippage.
- Agent API (`_Broker`): `buy/sell/short/cover(code, amount=, weight=, limit=None, valid_bars=None)` (`close` stays market). Limit orders skip the optimistic intra-tick position move (fill is uncertain); cross-tick de-dup relies on `pending()` (= query_stock_orders(cancelable_only)). xtquant mapping documented: buy/sell/short/cover/close <-> STOCK_BUY/STOCK_SELL/CREDIT_SLO_SELL/CREDIT_BUY_SECU_REPAY/market exit.
- Tests: `test_limit_order_fills_at_limit_when_bar_reaches_it` (09:30 buy limit 9.80, lag=2 activates 09:32, fills at 9.80 when 09:33 dips to 9.78 — exact limit, no slippage) and `test_limit_order_auto_cancels_when_unfilled` (limit 9.50 never reached -> expired_unfilled). Market-order/auction/pending/asof tests unchanged.
- Validation: full `unittest discover` 292 passing (skipped=2). Docs: environment_design §7.2, agent_design §5.2-5.3, prompt and template (main.py/README) document market + FIX_PRICE limit + valid_bars + the xtquant constant mapping; PROMPTS.md regenerated.

## 2026-06-27 Broker underlying interface aligned to xtquant (order book into SimBroker) (feat/rolling-asof)

- Scope: per the user's choice ("underlying aligned, wrappers stay"), the per-day order book moved from the replay engine into SimBroker, which now exposes the live xtquant lifecycle 1:1 while the ergonomic ctx.broker.buy/sell/short/cover/close(limit=, valid_bars=) wrappers are unchanged.
- broker.py: added `xtconstant` (STOCK_BUY/STOCK_SELL/CREDIT_SLO_SELL/CREDIT_BUY_SECU_REPAY/FIX_PRICE/MARKET_PEER_PRICE_FIRST) + internal action<->order_type maps; `WorkingOrder` (order_id, action, ts_code, volume/weight, price_type, price, remaining_bars TIF, is_auction). SimBroker gains `order_stock(order_type, stock_code, order_volume, price_type, price, *, weight, valid_bars, is_auction, reason) -> order_id`, `cancel_order_stock(order_id)`, `match_bar(trade_date, minute_key, minute_group, granularity)` (fills via execute; market=open+slippage, limit fills at the limit when [low,high] reaches it via `_limit_fill_price`, no maker slippage; TIF decrement + expired_unfilled auto-cancel). Renamed queries to xtquant verbs: `query_stock_asset` (was get_account), `query_stock_positions` (get_positions), `query_stock_orders(cancelable_only)` (was query_orders; now also returns the working book), `query_stock_trades(ts_code=None)` (was trades_for). `execute(order_id=)` carries the originating order id onto the fill/trade. A `TraderProtocol` (typing.Protocol) documents the shared surface a live QMTBroker (wrapping xt_trader) and SimBroker both implement. Module helpers `_bar_for_code`/`_open_price`/`_limit_fill_price` moved here (broker is the matching engine; avoids importing backtest_engine and the circular dep).
- main_ctx_engine.py: removed engine-local `_Order`/`_make_order`/`_fill_or_rest`/`_fill_price_for`/`_open_price`/`_working_orders`. The day loop keeps only the submit-lag scheduling (`incoming: activate_index -> [(action, is_auction)]`); at each real bar it submits due decisions via `broker.order_stock` (`_submit_order` maps action->order_type/price_type/price/valid_bars), calls `broker.match_bar`, and builds ctx pending from `broker.query_stock_orders(cancelable_only=True)` plus still-in-lag incoming (`_pending_view`), so de-dup holds across the whole decision->fill window. Day end cancels the remaining cancelable book.
- Callers updated (sed): query_orders->query_stock_orders in tools/backtest.py, backtest_engine.py, test_main_ctx_replay.py; trades_for->query_stock_trades and get_account/get_positions->query_stock_asset/query_stock_positions.
- Tests: new `test_order_stock_lifecycle_matches_xtquant_verbs` (order_stock -> order_id, query_stock_orders(cancelable_only), cancel_order_stock, match_bar fill-at-limit + expired auto-cancel, query_stock_trades/positions). All existing market/limit/auction/pending/asof tests pass unchanged — pure refactor.
- Validation: full `unittest discover` 293 passing (skipped=2). Docs: environment_design §7.1 now lists the wrappers + the SimBroker<->xtquant mapping table and TraderProtocol seam.

## 2026-06-27 Opus 4.8 read-only audit + fixes (feat/rolling-asof)

- An Opus 4.8 sub-agent audited all code + docs read-only against correctness, over-engineering, dead code, junk files, and docs-drift. Baseline 293 pass / 2 skip green. Verdict: the reworked execution model (next-bar/deferred fills, execution_lag_bars, per-day order book in SimBroker, FIX_PRICE limits, auction ticks, rolling as-of, pending() de-dup) is correct, internally consistent, and look-ahead-guarded with strong unit coverage; no HIGH-severity bug. PROMPTS.md in sync (only the volatile generated_at sample differs); git tree clean (no tracked caches/logs/data).
- Applied fixes:
  - Dead code (C1/C2): removed `backtest_engine._bar_execution_price` and `backtest_engine._minute_bar_for_code` — both grep-confirmed definition-only; the latter duplicated the live `broker._bar_for_code` used by match_bar.
  - Edge case (A1): `_day_tick_plan` now clamps `lag = max(1, min(execution_lag_bars, len(real_keys)-1))`. Previously a daily-fallback day (only 09:30/15:00) with auctions disabled and the default lag=2 dropped every continuous decision to `main_actions_unfilled` — a silent zero-fill behind the auction_enabled default. The clamp keeps >=1 (next-bar, no look-ahead) and <=n-1 (the first decision can reach the last bar), so 2-bar days still fill at 15:00. No existing test changed (all fixtures have >=3 bars or trade via lag-exempt auction ticks).
  - Template (A3/E4): `configs/agent_output_template/trading.py::example_swing_t` now early-returns on `ctx.broker.pending(ts_code)`, so the shipped 做T example is idempotent across the fill lag (it previously contradicted its own docstring warning).
  - Annotation (B1): `BrokerProfile.maintenance_warning_ratio`/`maintenance_withdraw_ratio` marked record-only (only `maintenance_closeout_ratio` drives forced close).
  - Hygiene (D1): `.gitignore` now excludes untracked `external_references/` (~10 MB vendored repos + a zip) and `check.md` scratch, preventing accidental `git add .` bloat. `git check-ignore` confirms both.
