...
| NL enum 输出合同 | 1–16 个唯一值，每值 1–64 字符 | 1 个检索轮 + 1 个强制最终回答；128 输出 token、每次检索最多 5 条、provider 每条证据片段最多 1,000 字符 |
...
| NL | 通用：3 个工具轮、3,000 输出 token；enum：1 个工具轮、128 输出 token | 通用检索默认返回 5 条、可请求 1 至 20 条；enum 路径最多 5 条并在工具预算后强制一次最终回答 |
