...
| `epochs` | 3 | Epoch 数；每个 Epoch 开始固定运行一次元学习 |
...
| `meta_learning_fold_interval` | 0 | 同一 Epoch 内的额外元学习间隔：`0` 保留基础行为；`N>0` 时每完成 N 个 Fold 且仍有下一 Fold，触发 Meta 并更新之后的 Taste（CLI `--meta-learning-fold-interval`） |
| `meta_memory_max_epochs` | 3 | 元学习原始记忆保留的最近 Epoch 数（每个 Epoch 只取当前会话之前最新一份 Meta Trace，`0` 关闭） |
| `meta_learning_directive` | 空 | 实验级元学习研究方向（CLI `--meta-learning-directive[-file]`；HITL 可按 Meta 会话经 `directive_override` 覆盖） |
| `fold_exploration_directive` | 空 | 实验级默认 Fold 探索主线；注入 Meta 与每个普通 Fold，并分别记录于 manifest/账本（创建表单或 CLI `--fold-exploration-directive[-file]`） |
...
以下值写入 run manifest。控制台暴露主要预算和回放旋钮；Timeview 和最终评估显式上限当前属于代码级配置，不在普通 CLI、HITL 表单或参数 API 中开放。开盘/收盘竞价时点是固定市场语义（09:15、09:25、14:57），不是实验参数。
...
| NL 检索 pattern 上限（常量） | 256 字符 | RE2/grep 语义；不支持反向引用和环视，越界或不支持时返回可修复工具错误；`event_filter` 的组合模式在 PIT 可见性之后应用且同样受此上限 |
| NL `event_filter` 合同 | 1–16 个 pattern，`lookback_days` 1–3660 | 仅单股请求；滚动窗口进入/移出都会使匹配 revision 失效，零匹配不调用 provider |
| NL enum 输出合同 | 1–16 个唯一值，每值 1–64 字符 | 单独使用：保留配置的推理模式，1 个检索轮 + 1 个强制最终回答；与 `event_filter` 同用：复用门控证据、关闭隐藏推理、1 次 provider 回答；512 输出 token、最多 5 条证据、每条 provider 片段最多 1,000 字符 |
...
| `trim_message_headroom` | 30 | 命中消息上限后批量裁剪预留的后续消息余量，避免逐轮重写模型前缀缓存 |
...
主对话与 NL 默认启用高强度推理模式（`reasoning_effort=max`）；compact 默认使用不启用推理模式的低成本模型（`environment_design.md` §2.4；CLI `--reasoning-effort`/`--no-thinking` 可覆盖）。
...
| NL | 通用：3 个工具轮、3,000 输出 token；enum：最多 1 个工具轮、512 输出 token | 通用检索和单独 enum 保留配置的推理模式；`event_filter + enum` 复用最多 5 条门控证据、关闭隐藏推理并禁用重复工具调用 |
...
| `--zero-rows-not-ready` | 盘前两融 4 个作业启用 | event_flow 专用：必需数据集空响应视为源端未就绪——零写入时退出码 75（无变更可重试契约，恢复 committed 世代、状态记 `not_ready`），有部分写入则提交并把空分区留给重试任务；不带旗标时空响应照旧 fail-fast |
...
| cron 详细日志保留期 | 14 天 | 清理未被当前 state 引用的 `logs/tushare/cron/tushare_cron_*.log`；state 只暴露实际存在的日志路径 |
| cron dispatch 轮转 | 5 MiB × 当前文件 + 2 个副本 | `logs/tushare/dispatch.log` 由 runner 进程锁定追加并按大小轮转 |
...
| 事件/打板删键缩水阻断阈值（常量） | >20 键且 >20% | 整分区重拉删键超阈值时 `blocked_shrink_overwrite`，保留旧分区 |
...
| 年化交易日数（常量） | 244 | 回放统计与风格归因（`replay/stats.py`、`replay/style.py`）共用的 A 股年化天数 |
| 数据质量报告 schema | v2（仅接受 v2） | 九字段统一 envelope；v1、无版本与未知版本均拒绝并要求重建 |
...
| QMT 本金上限（草案） | `CQ_MAX_PRINCIPAL` 环境变量 | 未设置时用账户总资产口径（`deployment_documentation.md` §4.1） |

持久化合同 schema 版本一览（每个版本号由正式常量定义并在标注的校验点生效；修改合同形状必须同批 bump 常量）：

| 持久化合同 | 常量（定义位置） | 当前值 | 校验/生效点 |
|---|---|---:|---|
| 数据质量报告 envelope | `autotrade.data_quality` schema v2 | 2 | 写前构建、文件读取、release pin/hash、snapshot 门共用 `validate_quality_report`，拒绝其他版本 |
| 修正账本事件 | `REVISION_EVENT_SCHEMA_VERSION`（tushare `common`） | 2 | `normalize_revision_event` 固定字段集校验 |
| raw generation 栅栏 | `GENERATION_SCHEMA_VERSION`（tushare `cron_update`） | 2 | 更新事务提交/脏标记 |
| 调度配置 | `SCHEDULE_CONFIG_SCHEMA_VERSION`（tushare `cron_update`） | 1 | `load_config` fail-fast，杜绝生产/消费漂移 |
| research release manifest | `_SCHEMA_VERSION`（environment `data/research_release`） | 2 | pin 发布与复用校验 |
| snapshot/replay 缓存格式 | `SNAPSHOT_CACHE_FORMAT_VERSION`（pipelines `config`） | 7 | 缓存键组成，不同版本互不复用（v7 起 manifest 携带 `dataset_columns` 供单位表生成） |
| runtime_env 合同 | `RUNTIME_ENV_SCHEMA_VERSION`（environment `sandbox`） | 2 | 镜像探针记录写入 |
| HITL control/status | `HITL_STATE_SCHEMA_VERSION`（pipelines `hitl_state`） | 1 | 记录写入 stamp |
| 实验账本记录 | `LEDGER_RECORD_SCHEMA_VERSION`（pipelines `ledger`) | 1 | `append` stamp |

raw parquet sidecar（`.meta.json`）刻意不带版本号：28 万+ 存量 sidecar 的宽容字段探测即是合同，追加版本字段需要全量重写且无消费端收益。
...
| `reasoning_effort` | `max` | Agent 与 NL 启用推理模式时的推理强度 |
| `no_thinking` | False | 关闭 Agent 与 NL 的模型推理模式 |
...
| 默认 Fold 探索方向 | `fold_exploration_directive`（空） | 新建实验弹窗的可选多行文本；自动注入 Meta 与每个普通 Fold，详情页会话指令只作追加；完整系统 Prompt 覆盖仍按原样替代自动装配内容 |
...
| 系统提示词预览 | `prompt-preview` | 批准前装配含最近已完成 Meta Taste、指令和等价占位事实的提示词；前置 Meta 尚未完成时拒绝生成，且不含 Sandbox 创建后才能生成的完整运行事实 JSON 或测试排程 |
| 系统提示词覆盖 / Fold 重跑 | `set_prompt_override` / `rerun_fold` | Fold 级整体覆盖（运行时原样使用、manifest 记录）；重跑仅限最新已记录 Fold且 worker 非 `launching`，自动模式转人工待批准，产物带 `__r<id>` 标签、held-out 自动重放 |
| 强制终止 / 恢复批准 | `terminate` | 当前计划会话未落账时撤销其 `approved_sessions` 项、保留指令与 Prompt 草稿；`auto` 同时转为 `manual`，恢复后重新等待预览与批准，已落账会话及模式不变 |
| 提前收官 / 回滚 | `skip_to_heldout`（可 `cancel_`） / `rollback_fold` | 跳过剩余 Fold 直接进 held-out（需 ≥1 冻结 Fold）；回滚到任一已记录 Fold（其后记录移除、唯一账本备份 `experiment_ledger.rollback_*.jsonl`、冻结产物归档 `_archive/`，worker 需已完全停止且非 `launching`） |
...
| 风格验证端点 | `style?run_id&prefix` | 原样返回该 run 落盘的 `results/style_<prefix>.json` rollup（回放时由宿主从冻结数据计算，Web 层零计算）。`style_valid.json` 只覆盖账本选定的验证窗口（多次验证尝试重复同一日期，不做跨尝试混合）。逐窗口完整版在 `results/<窗口>/style_analysis.json`（验证窗口 Agent 可读），紧凑 `benchmark` 块（基准/超额收益、β、市值倾斜）进回测 summary 与账本 Step 摘要 |
...
| 可关闭的预算 | 收尾窗口、盘外 tick 间距、元学习记忆 Epoch 数和元学习 Fold 间隔必须为有限非负数；Fold 间隔还必须为整数 | 实验配置构造时 |
