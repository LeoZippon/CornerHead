...
| `fold_exploration_directive` | 空 | 实验级默认 Fold 探索方向；注入每个普通 Fold 的自动装配系统 Prompt 并单独记录于 manifest/账本（创建表单或 CLI `--fold-exploration-directive[-file]`） |
...
| 默认 Fold 探索方向 | `fold_exploration_directive`（空） | 新建实验弹窗的可选多行文本；自动注入每个普通 Fold，详情页单 Fold 指令只作追加；完整系统 Prompt 覆盖仍按原样替代自动装配内容 |
