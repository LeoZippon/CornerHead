...
窗口参数可由实验 CLI、控制台或参数文件覆盖；未单独指定的数据域回退基础窗口。数据集清单和构造开关当前属于代码级配置。多数配置进入快照记录；`news_sources` 和 `news_window_months` 当前会影响构造，但未进入序列化配置记录，这是审计留痕限制。
...
| 参数 | 默认 | 作用 |
|---|---|---|
| `events_datasets` | margin、资金流、股东、回购、解禁、龙虎榜、打板、热榜和游资等 19 个数据集 | 决策快照的事件域成员；精确清单写入 snapshot manifest |
| `macro_datasets` | 国内宏观、利率、政策、全球指数、A 股指数、外汇、回购利率和美债曲线等 17 个数据集 | 决策快照的宏观域成员 |
| `text_datasets` | `anns_d`、`major_news`、`cctv_news`、`npr`、`research_report`、`report_rc`、`news` | 文本域成员 |
...
| `backtest_final_eval_max_seconds_per_decision` | 配置值 None；有效值默认 900 s | None 时按验证单决策上限的 3 倍派生；显式正值优先，仅作 frozen_eval 防挂死兜底 |
| `backtest_final_eval_max_seconds_per_trading_day` | 配置值 None；有效值默认 2700 s | None 时按验证单日上限的 3 倍派生；显式正值优先 |
...
| `nl_failure_policy` | `return_error_with_audit` | `return_error_with_audit` 返回可审计错误；`fail` 使回测失败；其他值在配置构造时拒绝 |
...
| 深圳开盘竞价校正倍率 | `00*.SZ` ×0.76、`30*.SZ` ×0.58 | 仅用于开盘竞价近似输入 |
...
| `meta_learning_host_proxy` | False（保留键） | 当前装配不读取此正向键；代理行为由网络和两个 `disable_*` 开关决定 |
...
| 控制台隐藏参数 | 路径、凭据变量名、`local_dev`、显式透传变量与代理二进制路径 | 不进普通表单，仅 API/`params.json` 可设 |
...
| HITL 参数 | 未知键直接拒绝；周期、类型、枚举和表单范围在创建阶段校验；逐 Fold GPU 表单范围为 1 至 16，但底层 Sandbox 只要求正数 | API/控制台创建或会话门控时 |
