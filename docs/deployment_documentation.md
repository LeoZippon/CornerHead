...
- `ops/qmt/qmt_monitor.sh start|stop|status` 管理 `scripts/live/qmt_live_monitor.py`（核心逻辑在 `src/autotrade/live/qmt_monitor.py`）：每 20s 经 `scp`（`QMT_SSH_DEST`，逐文件拉取，容忍远端缺文件）把 outbox 同步到 `data/qmt_live/`。
