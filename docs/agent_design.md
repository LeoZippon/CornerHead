...
Fold Agent 负责在单个 Fold 内读取允许的研究输入，探索和验证候选策略，并写出正式策略与可继承模型参数。Fold Agent 不负责准备数据、判定回放与 Broker 结果真相、冻结或编排实验、记录系统审计，也不能访问 Test/Held-out、真实凭据或实盘券商。独立 Meta Agent 只能额外读取已完成 Fold 的 compact Test 指标投影，边界见 Pipeline 设计 §3.2。
...
- 数据域用途、字段、单位、可见时间、窗口覆盖和路径权限，以本次运行的 `data_summary.json` 与清单为准；完整文件明细、单位表和包/CLI 清单不重复内联进系统 Prompt。单位按 `/mnt/artifacts/unit_reference.json`（当前快照全部可见字段的逐列单位表，入口经 `data_summary.json.unit_contract` 指针）以“文件 + dataset + 字段”查找并显式换算，不能按同名列猜单位；unknown 与表外字段的使用政策以 Data 文档 §1.2 的权威表述为准（unknown 仅限 dataset 内与量纲无关的运算；表外字段视为合同破损禁止使用）。Prompt 只保留查找纪律与该政策，不手工复制具体单位示例。
...
- Fold Agent 读取 Test/Held-out 数据或结果；Meta Agent 读取 compact 白名单投影之外的 Test 数据、排程、日志、NL/Broker 明细，或任何 Held-out 信息。
