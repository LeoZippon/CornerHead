...
完整 `ctx` 字段、可见性和时序合同见 `environment_design.md` §3.3；`ctx.broker` 策略接口与 QMT 映射见 `environment_design.md` §3.4。本节只说明 Fold Agent 如何组织策略代码。

策略实现时应按 Environment 合同使用这些 surface：`ctx.bars` / `ctx.price()` 只含当前 tick 可见行情，未来 bar 不可见；`ctx.asof_dir` / `ctx.asof_version` 应按版本缓存读取；`ctx.state_dir` 只能在 `ctx.substep` 内访问，写入暂存到 `ready_at` 后才并入可见目录；正式回放中 `ctx.model_dir` 只读，需要跨回测持久的数据应在回测前写入 `models/`。
