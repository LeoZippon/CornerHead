...
ctx.broker.buy/sell(ts_code, ...)         # 普通账户现金买入/卖出（long-only）；buy 可用 weight（目标账户初始资金比例）
ctx.broker.credit_buy/credit_sell(ts_code, ...)   # 信用账户担保品买卖
ctx.broker.fin_buy(ts_code, amount=None, weight=None, limit=None, valid_bars=1, reason=None)
                                          # 融资买入：开融资负债合约（本金+佣金计息），不动用现金
...
ctx.broker.cover/sell_repay(ts_code, amount=None, limit=None, valid_bars=1, reason=None)
                                          # 买券还券 / 卖券还款（净所得先息后本还融资），只接受 amount 股数
...
ctx.broker.transfer(amount, from_account, to_account, reason=None)
                                          # 两账户现金划转；融券冻结所得不可划出，信用有负债时划出须维保 ≥ 提取线
...
ctx.broker.stock                          # 普通账户视图 dict（cash/available_cash/total_assets/market_value）
ctx.broker.credit                         # 信用账户视图 dict（cash/available_cash、维保比例/保证金可用余额/负债/利息/额度）
...
`amount` 是股数（按 100 股，即 1 手，向下对齐），`weight` 是**下单目标账户**初始资金的名义比例。函数只表达意图；各账户现金、保证金可用余额、T+1 可卖余额、手数、涨跌停、停牌、融资融券标的池/额度/限价规则、负债利息、维保强平和划转提取线由 Broker 执行。每次实验同时运行普通 + 信用两个独立账户（互不担保）：普通账户 long-only（`buy`/`sell`），融资融券只在信用账户（`credit_buy`/`credit_sell`/`fin_buy`/`short`/`cover`/`sell_repay`/`direct_repay`）；两账户初始资金见 experiment facts 的 `broker_replay.stock_initial_cash` / `credit_initial_cash`，可经 `transfer` 划转。最大持仓数、单票权重上限和仓位集中度默认由 Agent 自行控制；只有 run config 显式设置 Broker 附加风控时才由 Broker 额外拦截（正式代码不得有死循环、网络访问或不可控写入，见 §3.1）。信用账户经济学（负债合约、利息、保证金公式、强平、划转）见 `environment_design.md` §3.3。
