...
- 代理与凭据：元学习默认启用代理别名机制；托管 XRay 有配置时默认启动，但命令默认直连。只有直连失败、明显卡顿或任务明确需要代理时，才把当前实验事实中列出的 active `AT_PROXY_*` 按单条命令临时映射为 `HTTPS_PROXY`/`ALL_PROXY`。token 只按 active 环境变量名使用，明文不得进入 prompt、产物、日志或账本。host/CLI 联网与代理配置的权威定义见 `environment_design.md` §2.1。
