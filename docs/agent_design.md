...
- 读取回测结果时优先看 `detailed_return.json`、`trade_intents.parquet`、Broker 事件、拒单统计和 NL 工具日志。
...
返回值必须包含 `trade_intents` 或 `trades`（`list[dict]` 或 `pandas.DataFrame`），把每只候选股票映射到一个交易策略函数。最小字段：

| 字段 | 说明 |
|---|---|
| `ts_code` 或 `code` | 股票代码（每只股票只能映射一个策略） |
| `trade_strategy` 或 `strategy` | 策略函数名，须在 `trading.py` 或 `main.py` 中定义 |

可选字段：

| 字段 | 说明 |
|---|---|
| `params` | 传给策略函数 `ctx.params` 的参数 dict；也可直接以同名列内联，如 `amount`、`price`、`percent`、`time` |
| `start_date` / `end_date` | 策略生效区间，格式 `YYYYMMDD` |
| `reason` | 简短交易理由 |
| `source_artifacts` | 使用的数据、函数或证据 ID |

### 5.3 策略函数

没有内置策略名。每个 `trade_strategy` 必须是 `trading.py`（或 `main.py`）中定义的函数 `def 名字(ctx): ...`。模板中的 `example_*` 函数只是可编辑样例，不是 Environment 支持的关键字。Environment 按分钟逐 bar 调用匹配函数，函数通过 `ctx` 调用 Broker 基础原语表达交易意图：
...
def example_swing_t(ctx):
    ...
    if ctx.broker.money >= ctx.cur_price * amount and ctx.cur_price < last_price * 0.95:
        ctx.broker.buy(amount)
    elif ctx.stock.position > amount and ctx.cur_price > last_price * 1.05:
        ctx.broker.sell(amount)
```

`ctx` 是纯交易回放上下文，暴露 `ctx.broker`、`ctx.stock`、`ctx.cur_price`/`cur_time`/`cur_date`/`bar`/`params`/`account`/`positions`；Broker 原语和 `ctx` 完整接口语义由 `docs/environment_design.md` 第 7 章定义。模型参数和 NL 只在 `main.py` / `candidate.py` 决策阶段使用；`workspace` 不进入正式策略 `context` 或逐分钟交易函数。回放函数也不能写 `output/` 或通过链接访问 `workspace` / `models`。

`amount` 是股数（按 100 股，即 1 手，向下对齐），`weight` 是初始权益名义比例。函数只表达意图；现金、做空保证金、T+1 可卖余额、手数、涨跌停、停牌和券源由 Broker 执行。最大持仓数、单票权重上限和仓位集中度默认由 Agent 在候选筛选、股数/权重和交易策略中自行控制；只有 run config 显式设置 Broker 附加风控时才由 Broker 额外拦截。函数应无死循环、网络访问或不可控写入。
...
宿主 Environment 负责启动 NL Sub Agent、提供 PIT `text_retrieve` 工具、构造公司上下文、调用 LLM provider 和日志落盘。API key 和 provider client 不进入 Sandbox。若策略需要数值分、标签或过滤条件，必须在 `main.py` / `candidate.py` / 决策 helper 中自行从 `result["content"]` 提取，并通过 `trade_intents.params` 传给交易函数。逐分钟交易函数不能调用 NL。NL 分析可能受到文本发布时间/入库时间误差、检索召回偏差、模型常识污染、自由文本解析不稳定和前视泄露影响；策略应按 PIT evidence 质量降权、过滤或放弃证据不足的结论。
...
- `output/main.py` 存在，并能返回结构化 `trade_intents` 或 `trades`。
