...
本文档记录 Fold Agent 的设计边界和维护口径。Agent 运行时直接读取的是系统 Prompt 和 `/mnt/agent/output/README.md`；本文档不作为运行时提示词，也不复制 Environment / Pipeline 的完整合同。PIT 数据、Sandbox、工具、Broker、NL 和回测由 `docs/environment_design.md` 定义；Step / Fold / Epoch / Held-out 编排、冻结和实验账本由 `docs/pipeline_design.md` 定义。
...
PIT、Timeview、数据槽、路径权限、工具 schema、Shell 边界、网络/代理、NL、Broker 和回测结果目录的权威合同见 `environment_design.md`。Agent 文档只保留使用原则：
...
- 数据域用途、字段、单位、可见时间、窗口覆盖和路径权限以本 run 的 `当前实验事实`、`data_summary.json`、run manifest、snapshot manifest 和 Environment 合同为准。
...
一个 Fold 内可以有多个 Step。Step 是同一个 Agent 会话中的一次“修改 -> 检查 -> 验证回测”迭代，不会重启 Agent，也不会创建新的对话上下文。
...
探索期允许有明确假设、可检验的自由探索。从配置的收敛阶段开始，Agent 应优先保持收益、Sharpe、回撤和可执行性，其次减少代码改动、文件数量和策略复杂度。临近 deadline 时，应恢复或保留当前最好且已完整验证的 `output`/`models`，尽快完成检查、验证或 `finish_fold`。
...
- `main(ctx)` 每个回放 tick 被调用一次，一次覆盖全市场；策略自己决定何时筛选、调仓、撤单和调用 NL。
...
- 建议把横截面筛选/开仓放在 `candidate.py`，把按 `ts_code` 的持仓管理、做 T、平仓和撤单放在 `trading.py`，由 `main` 在合适时点调用。
