...
```text
Timeview 可见性 = 行级 available_at <= 当前刷新节点 cutoff，且该节点已在仿真时钟下完成。
```
...
ctx.positions                             # 只读逐标的持仓快照列表（每行带 account）
ctx.price(ts_code), ctx.bar(ts_code), ctx.bars   # 仅当前 tick、PIT 可见的 bar（09:15 和 off-session 无价）
ctx.broker.buy/sell(ts_code, amount, ...) # 普通账户现金买入/卖出（long-only）
ctx.broker.credit_buy/credit_sell(ts_code, ...)   # 信用账户担保品买卖
ctx.broker.fin_buy(ts_code, amount, limit=None, valid_bars=1, reason=None)
                                          # 融资买入：开融资负债合约（本金+费用计息），不动用现金
ctx.broker.short(ts_code, ..., limit=P)   # 融券卖出：必须限价且申报价不低于参考最新价（uptick 规则）
ctx.broker.cover/sell_repay(ts_code, amount=None, limit=None, valid_bars=1, reason=None)
                                          # 买券还券（融券 T+1）/ 卖券还款（净所得先息后本还融资），只接受 amount 股数
ctx.broker.direct_repay(amount, reason=None)   # 直接还款（金额元，先息后本、最老合约优先；提交 tick 即时结算）
ctx.broker.transfer(amount, from_account, to_account, reason=None)
                                          # 09:14 前提交的当日盘前划转申请；09:14 统一确认，融券冻结所得不可划出，信用有负债时划出须维保 ≥ 提取线
ctx.broker.close(ts_code, account=None, reason=None)   # 市价平掉可平持仓；两账户同时持有时必须给 account=
ctx.broker.cancel(order_id, reason=None)                                           # reason= 为可选审计注记，driver 原样记录、不影响撮合
ctx.broker.stock                          # 普通账户视图 dict（cash/available_cash/total_assets/market_value；available_cash 扣 pending 买单冻结）
ctx.broker.credit                         # 信用账户视图 dict（cash/available_cash、维保比例/保证金可用余额/负债/利息/额度；可用项扣 pending 占用）
ctx.broker.debt_contracts(ts_code=None)   # 未了结融资/融券负债合约明细
ctx.broker.position(ts_code, account=None)   # 缺省跨账户净额；account= 看单账户
ctx.broker.pending(ts_code=None)          # 已提交但未成交/可撤的在途单；记录含 order_id/account/op_type/submitted_at/age_minutes/status，可能含 pending_stage
...
`ctx.asof_dir` 是逐 tick 滚动、按刷新节点门控的 PIT 视图；`ctx.asof_version` 只在视图真正滚动时变化，策略应按它缓存读取。`ctx.snapshot_dir` 是不随回放滚动的冻结研究基线。`ctx.bars` 只含当前 tick 已可见行情，未来 bar 不可见。

`ctx.state_dir` 是宿主托管的跨 tick 状态目录，只能在 `ctx.substep` 内访问；块内写入暂存到 `ready_at` 后才并入可见目录。需要跨回测持久的数据应在回测前写入 `models/`，正式回放中只通过 `ctx.model_dir` 读取。

Broker 约束摘要：`amount` 是股数，必须为正整数；普通 A 股 100 股整数倍，科创板 200 股起、之后 1 股递增。Broker 不做向下取整、超可卖量截断或单票 cap 自动压量，超约束直接拒单。仓位 sizing 由策略显式读取现金、价格和可卖量后自行计算，Broker 不接受 `weight` 下单参数。现金、保证金、T+1、手数、涨跌停、停牌、信用账户标的池/额度/限价规则、负债利息、维保强平和划转提取线由 Broker 执行；完整经济学与撮合语义见 `environment_design.md` §3。
