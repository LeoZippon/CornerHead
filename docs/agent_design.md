...
**成交时序与延迟**：订单不在决策 bar 内成交，而是延后若干 bar 进入撮合（市价/限价单、盘前竞价、`execution_lag_bars`、限价撮合与 `ctx.broker.pending()` 在途去重等完整成交规则见 `environment_design.md` §7.2）。推荐节奏：09:15 信息 tick（无价）筛选 + NL 并把目标写入 `ctx.state_dir`，09:25 tick（已知撮合开盘价）读取目标统一下单。把会消耗可观真实时间的重决策（筛选、模型推理、NL）包进 `with ctx.substep(name, budget_minutes=B):`：其中的单按声明的 `B` 延后成交；`B` 必须为正、tick 内 `name` 唯一，实测耗时超过 `B` 会中止本次回测（低报不可利用，高报拖累 P&L）；轻量决策给 `B ≤ execution_lag_bars` 的小值不改变成交 bar 仍获实时上限；不需建模延迟的琐碎逐 tick 代码可不包裹。声明预算的精确公式、`decision_max_sim_minutes` 上限和资源预算表见 `environment_design.md` §7.2。
...
ctx.substep(name, budget_minutes=B)       # 上下文管理器：声明决策延迟预算（B>0，tick 内 name 唯一，决定成交 bar，超时即中止回测）
