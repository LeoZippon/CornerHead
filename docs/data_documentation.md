...
特征、决策输入、回放和泄漏检查属于 `docs/environment_design.md`；Agent 的证据包和 LLM 输入属于 `docs/agent_design.md`；完整实验编排属于 `docs/pipeline_design.md`。
...
- [5. 原始数据时间可见性合同](#5-原始数据时间可见性合同)
  ...
  - [5.3 交给 Environment 的最小信息](#53-交给-environment-的最小信息)
...
- 调用 TuShare 和本地脚本下载 raw 数据。
- 保存每个文件的请求参数、来源和 hash。
- 记录单位、分区、分页、触顶、空响应和源端修正。
- 输出 6 个当前数据质量状态文件。
- 说明 raw 数据是否足以支持后续按时间可见性构造。
...
| 词 | 含义 |
|---|---|
| `raw` | TuShare 原始落盘层，路径通常是 `data/raw/<dataset>/...` |
| `sidecar` | 每个 parquet 旁边的 `.meta.json`，记录请求参数、抓取时间和 hash |
| `available_at` | 数据在回测或决策中最早可见的时间 |
| `status` | 当前数据质量审计结果 |
| `revision ledger` | 源端回写或本地/远端不一致事件账本 |
| `source cap risk` | 接口命中返回行数上限，可能被截断 |
| PIT | 按决策时点过滤未来信息；Data 层只保存支撑 PIT 的原始时间字段 |
...
财务 raw 层保留多版本、重复业务键和稀疏分区。Environment 会把它们构造成 `fundamental_events` 后再按 `available_at <= decision_time` 选择可见版本。
...
只有日期或月份的数据不得用于同日开盘决策；进入 Environment 后应优先使用精确发布时间或保守延后时间。
...
Data 层只保存文本 raw、发布时间、来源和 hash。进入 Agent 前，Agent evidence 层再生成 `evidence_id`、截断正文、实体映射和来源质量。
...
- cron 默认回看 30 天。
- 已存在且 sidecar 覆盖请求范围的分区跳过。
- 开放月份、开放年份和近期交易日会按配置强制刷新。
- 远端空响应不会覆盖本地非空分区，除非显式允许。
- 触发源端修正时写入 revision ledger。
...
| 日频行情与约束 | 最近 30 天强刷 | 差异写 revision ledger；`limit_list_d` 不稳定字段不进入冻结交易输入 |
...
| 文本数据 | 晚间滚动强刷，08:55 回补短新闻 | 重复推送保留 raw，Agent 层再去重 |
...
| `cn_nightly_full_audit` | 02:30 | 刷新 6 个顶层 status |
...
| `text_evidence_status.json` | 文本 raw tier |
...
- 文件是否存在、Parquet 是否可读。
- sidecar 是否存在，是否和 parquet 对齐。
- 是否有空文件、半成品、孤儿 sidecar。
- 预期分区是否齐全。
- 关键字段、业务键、日期字段是否可用。
- 是否命中分页行数上限。
- 单位和可见时间规则是否写入报告。
...
| 数据域 | 入口 | 输出 | 核心检查 | 特殊风险 |
|---|---|---|---|---|
| 基础研究 | `scripts/tushare/audit.py base --include-limit-list` | `base_research_status.json` | 基础维表、交易日、日频分区、财务多版本、跨表股票覆盖、单位 | `bak_basic` 起始较晚；日频表覆盖口径不同；财务重复键是 raw 语义 |
...
| 历史分钟线 | `scripts/tushare/audit.py intraday-by-date` | `intraday_minutes_status.json` | 按日文件、必需字段、重复 `(ts_code, trade_time)`、时间解析、09:30/15:00 条 | 正式 status 用本地分钟覆盖口径；严格 daily 覆盖只做专项排查 |
| 事件/资金 | `scripts/tushare/audit.py event-flow` | `event_flow_status.json` | 日频/公告分区、资金和事件单位、重复业务键、`share_float_complete` union | 解禁触顶风险；融资融券标的资格不等于券商券源 |
...
| 文本数据 | `scripts/tushare/audit.py base --include-text` | `text_evidence_status.json` | 月/日期/source 分区、时间字段、重复文本键、触顶风险 | 重复推送和转载是 warning；Agent evidence 层再生成 `evidence_id` |
...
专项报告只用于过程排查，不写入顶层 status。

## 5. 原始数据时间可见性合同

### 5.1 原始层原则

- raw 层尽量保留 TuShare 原始字段，不做特征化。
- raw 层不静默删除多版本财报、重复公告、稀疏事件和源端重复推送。
- 每个 parquet 必须有 sidecar，记录接口、请求参数、抓取时间和源数据 hash。
- raw 审计只说明数据是否足以支持 PIT，不声明某个策略无泄漏。
...
### 5.3 交给 Environment 的最小信息

每个数据域必须能提供：

- 来源：接口名、请求参数、分区路径、sidecar。
...
| 风险项 | 影响 | 当前处理 |
|---|---|---|
| 深圳 09:30 分钟条与开盘竞价接口口径不一致 | 历史竞价近似特征 | raw 不改写；Environment 生成校正字段：`00*.SZ` 乘 0.76，`30*.SZ` 乘 0.58，其他保持 1.0 |
...
| TuShare 可能回写历史数据 | 近期和部分历史分区 | cron 强刷滚动窗口并写 revision ledger；旧非空、新空默认不覆盖 |
...
| 财务多版本和公告日缺失 | 财务 PIT | raw 保留多版本；Environment 构造 `fundamental_events` 后选择可见版本 |
| 宏观发布时间不精确 | 月度/季度数据 | raw 使用保守可见时间，Environment 优先使用发布日程修正 |
| 文本重复推送和转载 | LLM evidence | raw 保留，Agent evidence 层按 hash 和时间过滤 |
...
它记录源端修正，不等于顶层 status。默认 `downstream_status=pending_review`，表示下游特征、缓存或实验结果是否需要重建尚未确认。
