...
当前快照可见字段的完整逐列单位表随每次快照生成为 `/mnt/artifacts/unit_reference.json`（Agent 入口经 `data_summary.json` 的 `unit_contract` 指针），人读规则视图为 `docs/units_reference.md`（生成文件，含状态定义与校验说明）。**unknown 与表外字段政策（权威表述，其余文档与 Prompt 均指向此处）**：`status=unknown` 的字段仅可在其所属 dataset 内做与量纲无关的运算（如排序、分位数），进入绝对阈值、单位换算或跨数据集算术前必须先显式核实单位；未出现在表中的字段视为数据合同破损，完全禁止使用并上报。观测数值范围只能用于校验，不能反向猜单位。完备性由两道校验保证：快照构建对每一列强制解析并对账 union 清单与物理 schema（漏归属即失败；events/macro 要求精确相等，财务域允许 schema 超前于窗口数据）；回归测试对 `configs/data/snapshot_columns.json`（每数据集最多抽样 5 个分区的供应商列清单并集，非全分区扫描）逐列解析——只出现在罕见历史分区的字段以运行时全列校验兜底。
...
`share_float_complete` 是解禁最终保留边界和日常更新的持久历史基线。活动 raw 分区按 `ts_code + ann_date + float_date + holder_name + share_type` 事件组整体替换或补充基线；组内按七个源字段去除完全相同的重复行，保留数量或比例不同的独立批次。归档过程文件不参与生产更新，基线缺失时普通增量更新直接失败。触顶分区使用 candidate 级补充；候选或源文件不可读时更新失败，未触顶 rescue 来源对完全相同的行提供来源优先级。如果某事件组的所有活动来源仍触及 6000 行上限，该组标记 `source_cap_risk` 并只与基线合并，不删除基线记录。
...
| 解禁 union | 每晚把近期新增/修订分区按事件组合并进 `share_float_complete` 完整基线 | 基线缺失直接失败；活动 raw 不承担历史重建 |
...
临时排查产物写入 `results/data_quality/process/`；处理后移到宿主归档根 `/Data/lzp/MacroQuant_Archive/data_quality/` 或删除。归档目录不参与生产数据更新；日常 `share_float_complete` 更新以现有完整表为历史基线，只合并活动 raw 中本轮新增或修订的业务键。
...
| `share_float_complete` 可能仍有触顶风险 | 解禁供给压力 | 专用入口补全并增量维护 union；exact-6000 标记 `source_cap_risk`；存在未触顶来源的活动事件组整体替换旧组，全部来源触顶的组只做保守并集；组内保留数值不同的独立批次 |
