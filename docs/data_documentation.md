...
- 每个 parquet 必须有旁路元数据，记录接口、请求参数、抓取时间、源数据 hash 和 `parquet_sha256`（发布前对暂存 parquet 字节计算；parquet 与 sidecar 都经 tmp+原子替换写入，写崩溃最多留下哈希不匹配的组合并被审计的抽样哈希校验检出，不会出现「新数据配旧元数据」被当作有效数据）。
...
| 2026-07-06 起日线量额含盘后定价成交 | 全部 A 股盘后固定价格交易（15:05–15:30）计入 `daily.vol/amount`，分钟线无对应 bar，分钟量和 ≠ 日线量 | 制度性口径差异，raw 不改写；依赖「分钟量和 == 日线量」的一致性检验/特征须知道该断点 |
