...
- [7. Broker、回放和做空规则](#7-broker回放和做空规则)
  ...
  - [7.2 main(ctx) 与分钟回放](#72-mainctx-与分钟回放)
...
7. 收集本分钟 `main` 发出的 Broker 原语调用，宿主 Broker 按延迟进入订单簿，逐 bar 撮合并强制约束。
8. 按分钟推进直到回放区间末日强制清仓。
...
交易逻辑全部由 Agent 定义。Environment 在 Sandbox 中启动一个常驻 `main(ctx)` 进程，按回放分钟逐分钟构造市场级 `ctx` 并调用一次 `main(ctx)`（一次覆盖全市场，而非每只股票一次）。`main` 自己决定时序：每分钟管理已有持仓、在选定时点筛选并开新仓，因此可以在任意分钟开/平仓。`ctx` 暴露的字段清单（`broker` 原语、行情、时间/账户、`nl`、`substep`、`asof_dir`/`snapshot_dir`/`model_dir`/`state_dir` 等）见 `agent_design.md` §5.3；本节定义其中由 Environment 决定的执行、延迟与可见性语义。
...
**决策延迟（Agent 声明式预算，LEAN 风格的确定性成交 + 松实时检查）。** 策略可用 `with ctx.substep(name, budget_minutes=B):` 包裹一段重决策（横截面筛选、模型推理、NL）。其中下的单成交 bar 顺延到 `决策分钟 + max(execution_lag_bars, ceil(B))`；`B` 由 Agent 声明并固定写入代码，故成交 bar 确定、跨 host 可复现（保证 accept/freeze 公平与回放一致）。Environment 不做任何延迟估算，只做两件事：（1）按声明的 `B` 排成交 bar；（2）实测每个 substep 的真实墙钟，一旦 `real_wall > B·60s` 立即抛 `BacktestError` 中止本次回测，并向 `backtest` 工具返回精确失败（substep 名、日期、声明 `B` vs 实测）。因此在已声明的 substep 内低报（声称快却跑得慢）硬报错、不可利用；高报延后成交、拖累 P&L。`budget_minutes` 必须为正：包裹但 `B=0` 会被 `ctx.substep` 拒绝（等同于无意义的不包裹写法）；同一 tick 内 substep 名必须唯一，重名同样被拒，以保证预算到订单的映射无歧义；轻量决策用较小的 `B`（如 0.5–1），由于 `B ≤ execution_lag_bars` 时成交 bar 不变，几乎不影响成交时点却仍获得逐块实时上限。该模型是**协作式**的：fail-fast 只保证“已声明的 substep 不会低报”，并不强制 Agent 包裹每段重决策；不包裹的逻辑按默认 lag 成交、无逐块声明上限（其真实墙钟仍受系统的 `backtest_max_seconds_per_decision` / `backtest_max_seconds_per_trading_day` 硬上限兜底，但这些不改变成交 bar）。因此如实包裹重决策、跨 Fold 拟合稳定平均预算属于元学习信号驱动的协作约束。声明的 `B` 受 `decision_max_sim_minutes` 上限约束：超过则该决策**可复现地**错过成交（记 `decision_too_slow`）。NL 由宿主串行服务（`_serve_nl_requests`），substep 真实墙钟约等于其中各 NL 时延之和，Agent 据此设定 `B`。盘前竞价 tick 同样适用该公式：09:15 的 `activate_index=0`、09:25 的 `activate_index=1`，故 `ceil(B) ≤ lag_floor` 的小预算不改变竞价成交（仍按 §7.2 盘前规则成交于 09:30/09:31）；较大预算（`ceil(B) > lag_floor`）会像连续单一样把竞价单的成交 bar 顺延到更晚（`price_label` 仍为 `auction`）。因此不要给盘前竞价单设置较大的 substep 预算。`ctx.substep` 是执行无关的，live QMT 控制器可把各 substep 作为真实异步任务运行、并以同样的预算超限作为错误。

**回测独立计时、成本上限与可观测性。** `backtest` 作为独立计时工具，其墙钟时间不计入 Fold 推理 deadline（`runner.py` 把回测耗时回补到 deadline），但单个 Fold 最多 `max_backtests_per_fold` 次回测，超出返回 `backtest budget exhausted`。两道随回放天数伸缩的真实墙钟硬上限（替代固定总上限）：单个决策（一次 `main(ctx)` tick，含其内 NL）超过 `backtest_max_seconds_per_decision` 由 `MainPolicyRunner` **立即杀掉**驱动并抛 `BacktestError`（该 tick 的硬截止，不再因 NL 重置）；某交易日累计 `main(ctx)` 计算超过 `backtest_max_seconds_per_trading_day` 在引擎层中止回放。完整验证触发任一上限即不可接受/冻结，迫使 Agent 缓存重计算、压低调仓/图构建成本。**这两道紧上限是真实墙钟、随机器负载浮动，故仅约束 Agent 迭代的 `mode="valid"` 验证回测**；最终评估（每个 Fold 的冻结 `test_000` 与 held-out，均 `mode="frozen_eval"`）改用更宽松的防挂死兜底 `backtest_final_eval_max_seconds_per_decision` / `backtest_final_eval_max_seconds_per_trading_day`（默认 900s / 3000s）：已在验证阶段满足紧上限的策略必须能跑完其最终评估，accept/held-out 结果不应因负载浮动而不可复现。该兜底只为杀掉真正的挂死——基于仿真时间的预算无法做到（单 tick 内死循环消耗 0 仿真分钟却占用无限墙钟，只有墙钟兜底能拦），且不作为接受门槛。**有意不设固定总上限**：单次回放的总耗时上界即 `交易日数 × backtest_max_seconds_per_trading_day`，随回放长度自然伸缩，无需另设一个不随长度变化的总墙钟上限。Environment 不预测回放耗时；Agent 用小 `replay_window` 试探得到的 `replay_wall_seconds` / `replayed_trade_days` 自行外推完整运行成本。回测可观测性：开始时发 `backtest_start`、回放期间按节流（≥30 天或≥30 秒）发 `backtest_progress` 心跳（含进度/已用时/累计订单数）、结束/中止保证有一条终止 `backtest` 事件（含外部中止的 `status="aborted"`）；summary 另含 `started_at`、`replay_wall_seconds`、`replayed_trade_days` 和逐 substep 的 `substep_runtime`（count/total_real_wall_s/max_real_wall_s）。
...
| substep `budget_minutes`（fail-fast） | Agent 声明（`B>0`，tick 内唯一） | substep 的真实墙钟上限 + 成交延迟 |
| `decision_max_sim_minutes` | 60min | 声明预算 `B` 的上限（超过即错过成交） |
