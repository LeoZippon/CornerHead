...
`ctx.asof_dir` 中的 parquet 域和文本视图均需同时满足：覆盖该域或该 `dataset` 的刷新节点已完成，且行级 `available_at <= visibility_cutoff`。`text_index` 与 `text_library` 也按同一文本刷新节点门控；`ctx.nl()` 复用同一仿真时钟和文本可见性。
...
- 决策在某根 bar 产生后，订单于其后第 `execution_lag_bars` 根 bar 起进入撮合；默认 2，表示“一根算/报单延迟 + 下一根成交”，如 09:35 决策、09:37 起成交。`execution_lag_bars=1` 表示紧邻下一根 bar 起撮合。
...
- 提交延迟队列中的订单被撤销时记录 `order_cancelled(pending_stage="submit_lag")`。
- 已进 Broker 订单簿但未成交的订单调用底层 `cancel` 移除；已经在当前激活 bar 成交的市价单不能事后撤销。
...
- 限价单：`limit=P`，对应 `prType=11` 指定价；自进入 bar 起最多 `valid_bars` 根 bar 有效，不计滑点。
- 限价买入/补券：`open<=P` 时按 `open` 成交，否则 `low<=P` 时按 `P` 成交。
- 限价卖出/融券卖出：`open>=P` 时按 `open` 成交，否则 `high>=P` 时按 `P` 成交。
- 限价窗口内未触及则自动撤单并记录 `order_cancelled`。
...
- 融券卖出必须限价申报，市价 short 在 `passorder` 即拒（`slo_sell_requires_limit_price`）。
