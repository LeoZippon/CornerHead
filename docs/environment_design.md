...
4. 固定 `AT_SNAPSHOT_DIR=/mnt/snapshot`、`AT_AGENT_OUTPUT_DIR=/mnt/agent/output`、`AT_MODEL_DIR=/mnt/agent/models`，在 Sandbox 子进程执行 `output/main.py`。
5. 若决策代码在生成 `trade_intents` 前调用 `at_tools.nl()`，通过 JSONL 文件 RPC 请求宿主侧 NL 服务。
