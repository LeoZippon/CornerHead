...
| `backtest` | 执行 `output/main.py` 并回放交易；Agent 可传的业务参数只有 `replay_window`（探针不执行 NL 内容生成，只返回匿名 substep 成本和订单生命周期统计，不产生收益、成交或归因文件） | 消费并校验当前 snapshot；每次调用创建唯一结果目录 |
...
| 回放规模 | `replayed_trade_days`、`total_ticks`、`intraday_ticks`、`offsession_ticks`、`decision_calls`、`strategy_action_count` |
