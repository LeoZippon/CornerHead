...
|---|---|---|---|
| 日频数据可见性合约 | `src/hl_trader/environment/data/contracts.py`、`data/raw/{daily,daily_basic,adj_factor,stk_limit,suspend_d}` | 定义日频分区在决策时点是否可见 | 只读取 `available_at(partition_date) <= decision_time` 的分区，并按各数据集自己的可见日期拼接 |
...
| 标准单位归一化 | `src/hl_trader/environment/features/units.py` | 统一 `daily.parquet` 中成交量、成交额、比例类字段单位 | 只做单位规范化，不生成 alpha 因子；转换记录写入 `manifest.json` |
...
若使用本机 XRay/代理客户端，先在宿主导入配置并启动代理端口，再把宿主 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY` 或 `NO_PROXY` 配好。`--meta-learning-host-proxy` 不把这些标准变量直接注入容器，而是映射为非标准别名 `MQ_PROXY_HTTP`、`MQ_PROXY_HTTPS`、`MQ_PROXY_ALL`、`MQ_PROXY_NO_PROXY`，因此 Agent 默认仍然直连。若 direct GitHub/HuggingFace/PyPI/npm 访问卡顿或失败，Agent 可在单条 shell 命令前临时启用代理，例如 `HTTPS_PROXY="$MQ_PROXY_HTTPS" ALL_PROXY="$MQ_PROXY_ALL" hf download ...`。`bridge` 网络下本地回环代理地址会改写为 `host.docker.internal`；`--meta-learning-host-proxy` 会自动添加 host gateway。仓库、manifest、trace 和 system prompt 只记录环境变量名、网络模式和使用边界，不记录 token、订阅链接或代理凭据。GitHub token 由用户用 GitHub/`gh auth` 创建；流程不会自动生成第三方 token。
...
4. 固定 `MQ_SNAPSHOT_DIR=/mnt/snapshot`、`MQ_AGENT_OUTPUT_DIR=/mnt/agent/output`、`MQ_MODEL_DIR=/mnt/agent/models`，在 Sandbox 子进程执行 `output/main.py`。
5. 若决策代码在生成 `trade_intents` 前调用 `mq_tools.nl()`，通过 JSONL 文件 RPC 请求宿主侧 NL 服务。
6. 规范化 `trade_intents`（候选→策略函数映射）。
7. 校验股票池、代码唯一性、策略函数可解析、`params` 和生效区间。
8. 启动策略函数进程，按分钟逐 bar 调用每只股票的策略函数，宿主 Broker 执行原语并强制约束。
9. 写结果、Broker 事件、NL 工具日志、策略/model hash 和 manifest 摘要。
...
策略函数内可用的 `ctx.broker` 接口：

| 接口 | 作用 |
|---|---|
| `buy(amount\|weight)` | 多头买入（建仓或加仓） |
| `sell(amount)` | 卖出多头可卖（T+1）份额 |
| `short(amount\|weight)` | 融券开空 |
| `cover(amount)` | 买券还券（平空可平份额） |
| `close()` | 平掉当前可平持仓 |
| `money` / `cash` | 当前乐观现金视图 |
| `account` / `positions` | 当前账户和持仓快照 |

宿主侧 `SimBroker` 另提供 `get_account()`、`get_positions()`、`query_orders()`、`trades_for(code)` 等查询接口，用于回测统计和向 `ctx.stock.trades` 注入历史成交。

`amount` 是股数（按 100 股，即 1 手，向下对齐），`weight` 是初始权益的名义比例。所有拒单、部分成交、T+1 阻挡和强制平仓事件必须记录。

### 7.2 Agent 策略函数与分钟回放

交易策略全部由 Agent 定义，没有内置策略名。模板中的 `example_*` 函数只是普通样例代码，不是 Broker 或 Environment 识别的关键字。`main.py` 返回的 `trade_intents` 把每只候选股票映射到一个 `trade_strategy` 函数名（定义在 `trading.py` 或 `main.py`），可附带 `params` 和生效区间，每只股票只能映射一个策略。

Environment 在 Sandbox 中启动长驻策略进程，按分钟逐 bar 回放：对每个 due 股票的当前 bar，用单个 `ctx` 调用其策略函数 `def 名字(ctx): ...`。`ctx` 暴露：

- `ctx.broker`：`buy/sell/short/cover/close`、`money`/`cash`、`account`、`positions`。
- `ctx.stock`：`code`、`price`、`position`（有符号股数）、`trades`（该股历史成交，每笔含 `price/side/quantity/date`）。
- `ctx.cur_price`、`ctx.cur_time`（"HH:MM"）、`ctx.cur_date`、`ctx.bar`、`ctx.params`、`ctx.account`、`ctx.positions`。

模型参数和 NL 工具只在 `output/main.py` 生成交易意图的决策阶段可用；`workspace` 不作为正式决策 `context` 字段传入。逐分钟策略函数不接收 `ctx.model_dir`、`ctx.workspace_dir` 或 `ctx.nl`；需要的模型/NL 结果必须提前写入 `trade_intents.params`。回放进程只读加载 `output/` 中的策略代码，禁止写 `output/`、创建软/硬链接，且按真实路径阻断经链接访问 `workspace` 或 `models`。

函数调用的原语作为意图回传宿主，宿主 Broker 执行下单、成交、拒单、约束和审计；Broker 与 Sandbox 之间的隔离边界不变（策略只表达意图，不直接改账户）。

分钟回放是默认口径：有非空 `intraday_1min.parquet` 时按真实分钟 bar 推进；缺失分钟数据的日期/股票，或缺少必要收盘分钟 bar 时，按日线合成 09:30/15:00 两根 bar 作为退化 fallback。成交价使用当前 bar `close` 再叠加滑点。回放区间最后一个交易日保留为剩余持仓的强制清仓日。
...
results/<phase>_<idx>/
  ...
  trade_intents.parquet
  strategy_metadata.json
  candidates.parquet        # 若 main.py 返回 candidates
  nl_tool/                  # 策略调用 nl() 时有内容
```
