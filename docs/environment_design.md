...
- 提供 Agent 常用工具（只读检索、文件读写、Sandbox shell、explore/web_search 等）、修改约束检查、回测入口、NL 服务和模拟 Broker。
...
| 数据域 | Snapshot 文件 | 主要来源 | 输出边界 | 窗口配置 |
|---|---|---|---|---|
| `daily` | `daily.parquet` | 日线、每日指标、复权因子和交易日历 | 日频行情、横向排序输入和 Broker 回放参考 | `daily_window_months`，缺省回退 `window_months`；默认最近 21 个月 |
| `intraday_1min` | `intraday_1min.parquet` | 1 分钟线和交易日历 | 日内策略、开收盘和做 T 研究 | `intraday_trade_days`；默认最近 21 个交易日 |
...
本节是各数据域可见性判定的权威位置；§1.1 只定义文件归属、来源、输出边界和准备窗口长度。Snapshot 构建时先按准备窗口取候选数据，再按下表裁剪到 `decision_time` 前已经可见的数据。

| 数据域 | PIT 可见性合同 |
|---|---|
| `daily` | 日线、每日指标、复权因子、涨跌停和停牌等日频分区按各自可见日期进入；决策时点之后的交易日行情、成交、估值和约束字段不能进入。盘中当日实时行情走 `ctx.bars` / `ctx.price`，不写入持久 snapshot。 |
| `intraday_1min` | 开盘前不含当日分钟线；盘中只包含已经收盘且 bar close 时间不晚于 `decision_time` 的分钟线。历史 09:30 开盘竞价近似字段按 §1.4 的校正规则处理。 |
| `fundamentals` | 财报、财务指标、分红、业绩预告/快报、披露计划和主营构成按公告日、披露日、报告期和版本字段选择已可见版本；公告日字段优先使用 `f_ann_date` / `ann_date` 等明确披露字段。缺少公告日的数据必须有保守回退规则，否则排除。主营构成等自身缺少公告日的行，可以按同股票、同报告期已披露财报事件回退可见。 |
| `events` | 资金流、两融、股东、回购、解禁、大宗交易、龙虎榜等异构事件按 `available_at` 过滤；T+1 或盘后数据使用盘后或下一交易日的保守可见时间，事件发生日不等于可见日。 |
| `macro` | 有明确发布时间的宏观、政策、利率、全球事件和跨市场数据按发布时间过滤；只有统计期或报告期而无发布时间的数据，按数据集配置的保守可见时间进入；没有可靠回退规则则排除。 |
| `text` | 正文检索必须引用已经可见的 `text_id` 或 `source_hash`；公告、新闻、研报和政策文本按 `anns_d`、`rec_time` 或文本管线给出的 `available_at` 门控。回填采集的历史文本不得早于其可证明发布时间可见。 |
| `universe` | 只包含决策日在市股票：`list_date <= decision_date` 且未在决策日前退市；不得用当前上市名单回填历史。 |

Agent 可见输入是 `/mnt/snapshot` 下的 PIT 裸数据窗口：`daily.parquet`、`intraday_1min.parquet`、`fundamentals.parquet`、`events.parquet`、`macro.parquet`、`text_index.parquet`、`text_library/` 和 `universe.parquet`。Snapshot 只做 PIT 可见性过滤、标准单位归一化、跨 raw 表同键拼接和交易约束字段对齐；不预构建 alpha 因子、滚动收益、均线、波动率、综合分数或候选排名。`limit_list_d` 等研究标签不进入预计算 alpha 列；Agent 如需使用，必须在可见窗口内自行解释。

**PIT 支撑机制。** 财务多版本可见性来自 `fundamental_events` PIT 索引：行级 `available_at` 取公告日（优先 `f_ann_date`/`ann_date`）当日 **18:00**；索引由 `scripts/data/build_pit_events.py`（cron job `cn_nightly_pit_event_build`）构建并审计，启用 fundamentals 时 `fundamental_events_status.json` 缺失、非法或 `status=error` 会直接阻断 snapshot 构造（状态文件与索引布局见 `data_documentation.md` §3.1）。按 `available_at` 门控的配置数据集若缺少该列，snapshot 构建必须报错而非静默放行；构建完成后 manifest 记录 `build_profile` 与 `data_profile` 供审计。


### 1.4 单位与特殊口径

单位合同（实现：`src/autotrade/environment/features/units.py`；raw 侧单位见 `data_documentation.md` §1.2）：
...
| 运行时模块 | 镜像把可信宿主侧运行时模块烤入 `/opt/at_runtime/`：常驻 `main(ctx)` 驱动 `main_ctx_driver.py`（Python 标准库实现，不依赖 `broker_core`/pandas）。substep 内 broker action 是延迟提交计划、由宿主 `SimBroker` 撮合并约束，故 driver 不在沙箱内投影成交，`ctx.broker` 的现金/持仓/可用买力只反映已成交状态。基础镜像构建上下文为仓库根（`docker build -f ops/docker/sandbox.Dockerfile .`）；`/opt/at_runtime` 路径须与 `executor.CONTAINER_RUNTIME_DIR` 一致，改动该模块需重建基础镜像 |
...
| `shell` | 在 Sandbox 内读数据、写 `workspace`、`output` 或 `models`；元学习开放网络时可运行 `git`/`pip`/`npm`/`hf` | 不是宿主 shell；普通 Fold 拦截安装/下载/联网入口；可用 `max_output_chars` 和 `timeout_seconds` 主动缩小内联输出和单次运行时间；长输出落盘并记录路径 |
...
所有工具 trace 都记录当前 `tool_spec` 的 `schema_version` 和 `result_policy`。`shell` 作为工具之一，额外记录 `command_kind`，取值如 `read`、`list`、`search`、`write`、`install`、`network` 或 `unknown`；该字段用于审计和后续统计，权限判断仍由 Sandbox、路径 guard 和阶段策略执行。
...
diff 基准必须可信才能测量修改量：`modification_check` 先校验父策略产物 hash 与 run manifest 一致（初始模板则校验 `initial_template_hash`），不一致直接报错。父模型参数走对称规则——当父模型参数目录非空时，manifest 必须提供 `parent_model_artifact_hash` 且与实际一致；只有空的父模型目录才允许用计算出的空目录 hash 作基准。任一基准不可信都 fail-fast，不会静默继续。

产物变更后必须重新检查。失败时只能缩小正式修改后重试。

`finish_fold` 在锁定前先做与 Pipeline 冻结同口径的把关：当前 `output`/`models` hash 必须已有一次成功的完整验证回测（`replay_window` 调试回放不算），否则直接拒绝并返回可修复原因（重跑完整 `backtest`，或先恢复最近已完整验证的 Step）。随后确认最近一次修改检查仍匹配当前 hash，并执行轻量合同检查，确认当前产物可加载且 `main(ctx)` 存在。成功后，Runner 锁定 `output/` 和 `models/` 写入，Pipeline 再复核验证结果和当前策略/model hash 是否一致。
...
**NL 调用配额（成本，与延迟分开）。** 每次回测的 `ctx.nl()` 总配额默认按 `nl_max_calls_per_decision_day`（系统设定的日均上限）× 决策天数计算（一个日均预算），可由可选 `nl_max_calls_per_backtest` 进一步收紧（取 min）。超出后 NL 服务向策略返回 `budget_exhausted` 错误，策略需自行降级。该配额只约束调用次数（成本）；调用耗时由 §3.2 的声明式延迟预算（`ctx.substep`）单独约束。
...
- 元学习 `web_search` 由宿主侧工具执行；可用引擎写入 manifest，Agent 在 action 中选择 `engine`，并用 `perspective` 标记研究视角。启用搜索时，Runner 要求三类研究视角各有一次非空成功检索后才允许 `done`（视角定义与 Taste 收敛要求见 `pipeline_design.md` §3.1）。
- 元学习可由实验配置显式开放 Sandbox shell 网络做工作区内探索，并通过 `workspace/sandbox_environment.json` 申请后续 Fold 继承的依赖、由 Pipeline 构建派生镜像；该能力不替代 `web_search` 的三视角要求，也不开放给普通 Fold。网络/代理配置见 §2.1；请求 schema、构建烟测、镜像 tag 持久化与 GC 见 `pipeline_design.md` §3.1。
...
| 接口 | 作用 |
|---|---|
| `buy(ts_code, amount\|weight, limit=None, valid_bars=1, reason=None)` | 多头买入（建仓或加仓）；`limit` 为限价单 |
| `sell(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 卖出多头可卖（T+1）份额 |
| `short(ts_code, amount\|weight, limit=None, valid_bars=1, reason=None)` | 融券开空 |
| `cover(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 买券还券（平空可平份额） |
| `close(ts_code, reason=None)` | 平掉该股可平持仓（恒市价） |
| `cancel(order_id, reason=None)` | 撤销 `pending()` 返回的未成交委托 |
| `position(ts_code)` / `pending(ts_code=None)` | 该股有符号持仓股数 / 在途/延迟提交单；`pending()` 无参返回全量 |
| `money` / `cash` | 当前现金视图，每 tick 反映已成交结果（含佣金/滑点）；未成交计划不改变它 |
...
`amount` 是股数（按 100 股，即 1 手，向下对齐），`weight` 是初始权益的名义比例。所有下单/撤单原语都接受可选 `reason=`（默认 `None`）审计注记，Sandbox driver 原样记入 Broker 事件、不影响撮合。`buy`/`sell`/`short`/`cover`/`close` 返回可用于撤单的 `order_id`；`pending()` 记录包含 `order_id`、`submitted_at`、`age_minutes`、`status`，并可带 `pending_stage`（如 `substep_delay` / `submit_lag`）等字段。`substep_delay` 阶段的 `age_minutes` 从动作生成 tick 起算；ready 后进入 `submit_lag` 阶段则从实际提交 tick 起算。所有拒单、撤单、部分成交、T+1 阻挡和强制平仓事件必须记录。
...
每个 parquet 域是一个普通 parts 目录，用 `pd.read_parquet(ctx.asof_dir / "<域名>")` 读出拼接：part 0 是该域的冻结研究快照（硬链入，零拷贝），后续 parts 是 write-once 的回放增量，只在仿真时钟跨过覆盖该域的节点时追加；因此 09:20→次日 02:05 一整段没有节点完成，视图被冻结、零重建。`ctx.asof_version` 在视图滚动时自增，策略可缓存一次读取、仅在版本变化时重算。`ctx.snapshot_dir` 仍是冻结的研究基线。视图开关是 `timeview_enabled`（默认开）。
...
宿主 Broker 据此执行下单、撤单、成交、拒单（现金/做空保证金/T+1/手数/涨跌停/停牌/券源）、约束和审计；隔离边界不变（策略只表达意图）。`ctx.positions` 只反映已成交持仓；在途（已报未成）单经 `ctx.broker.pending(ts_code)` / `ctx.broker.pending()` 暴露（对应 `query_stock_orders(cancelable_only)` 加提交延迟队列），策略据此对在途代码跳过重复下单或撤销过期委托。
...
`detailed_return.json` 至少包含总收益、long/short 收益、年化收益、Sharpe、最大回撤、胜率、turnover、订单状态、拒单统计、费用、借券费、权益曲线、逐笔平仓/减仓和 Broker 事件。回测 summary 另记 `state_staged_writes`/`state_unmerged_writes` 计数，以及 §3.2 的 `phase_seconds` 和 tick 计数。
