...
`main` 每个 tick 发出的原语作为意图回传宿主，采用**次一根 bar 成交**：在某根 bar 上决策的单，于下一根 bar 的开盘价成交，不在同一根 bar 内成交，杜绝 bar 内前视（09:35 触发的单成交于 09:36 开盘）。宿主 Broker 据此执行下单、成交、拒单、约束和审计；隔离边界不变（策略只表达意图，不直接改账户）。`ctx.positions` 只反映已成交持仓，故每个入场意图应在某个决策 tick 下一次，跨 tick 的在途意图由策略自己在 `ctx.state_dir` 跟踪，避免下一根成交前重复下单。

分钟回放是默认口径：有非空 `intraday_1min.parquet` 时按真实分钟 bar 推进；缺失分钟数据的日期/股票，或缺少必要收盘分钟 bar 时，按日线合成 09:30/15:00 两根 bar 作为退化 fallback。成交价使用成交 bar（决策 bar 的下一根）的 `open` 再叠加滑点。回放区间最后一个交易日保留为剩余持仓的强制清仓日；末根 bar 上的新意图无下一根可成交、将被丢弃。
