...
- 归一执行关键的日频字段；为其他研究域保留并记录源单位口径，同时记录覆盖、版本、hash 和转换规则。
...
- 构建全程对数据更新 runner 的锁持共享 flock，manifest 记录 raw 世代戳；构建期间数据湖世代变化立即失败。
...
- Agent 可见的数据摘要提供各视图的文件规模、行数、日期覆盖和大表访问提示。主决策视图另给关键列和空值计数；训练与验证槽只给规模和覆盖。
...
- 验证回放槽在研究期物理可读，Agent 可能看到该文件；可用于决策的分红消息仍必须按公告时间门控。
...
- `09:25` 只暴露开盘价，不含日内高低价和成交量；此时提交的订单从首根连续 Bar 撮合，并按连续交易计滑点。
- 开盘价只取开盘时段的首根 Bar。若证券当日首笔行情更晚出现，09:25 视图中必须缺席，直到真实 Bar 到达。
...
1. **验证阶段墙钟 fail-fast**：验证回测中，substep 真实墙钟超过 `B·60s` 时立即失败，并返回步骤、日期、声明预算和实测耗时。低报预算不可利用。
...
- 逐决策墙钟上限和每次回放重置始终强制；substep 实测超时与覆盖检查只在验证回测强制。冻结测试和 held-out 关闭这两项检查，但仍受更宽松的逐决策与每日硬上限。

状态和产物约束：

- `ctx.state_dir` 与 `ctx.broker` 原语始终要求位于 substep。验证回测还会用 `main(ctx)` 总耗时减去 substep 耗时，拒绝实质未包裹计算；冻结评估不执行这项覆盖检查。
...
- 默认按验证阶段对应上限的固定倍数派生；显式配置可以覆盖，当前值见 [参数参考](parameters_reference.md#3-回放执行与预算)。
- 这些上限只用于终止挂死，不是策略接受门槛。
- 策略已在验证阶段满足更紧上限，最终评估的放宽只用于降低机器负载波动造成的非确定性失败。
...
- 冻结测试和 held-out 在 Agent 会话结束后运行，因此 Agent 只能读取验证期归因。
