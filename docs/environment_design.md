...
| `universe` | `universe.parquet` | `stock_basic` 的 L/D/P 状态和上市日期做在市过滤，`name` 列取 `namechange` 的决策日 as-of 名称（缺记录为 null，绝不回填当前名），`delist_date` 过滤后删除（幸存者的退市都在决策日之后，属未来信息），叠加 `index_member_all` 的 as-of 申万一级行业 | 决策日在市股票池，避免幸存者偏差与未来名称/退市泄漏 | 不使用月份窗口；按决策日在市口径生成 |
...
- **标的池**：`margin_secs` 原始数据不区分担保品/融资/融券标的，当前研究用同一集合（成交日逐日刷新）近似门控 `credit_buy`（`margin_secs_not_collateral`）、`fin_buy`（`margin_secs_not_finable`）与 `short`（`margin_secs_not_shortable`）；回放槽有逐日数据但缺当日集合时 fail-closed 拒 `margin_secs_data_missing`（真实名单逐日发布，沿用陈旧集合会高估可得性）；`theoretical_short` 模式同时豁免三者。
...
| 模式 | 规则 |
|---|---|
| `proxy_margin_secs` | 研究近似：信用账户开仓标的按成交日的真实 `margin_secs`（回放槽的逐日映射）门控，逐日数据缺当日集合时 fail-closed 拒单（`margin_secs_data_missing`）；仅当回放槽完全没有 margin_secs 域时才回退冻结决策日快照（记档的退化模式）。不代表真实担保品池、融资池或融券券源 |
| `broker_inventory` | 未来接入真实券源；应要求交易所融资融券资格、真实券源、数量覆盖和合约费率均可见 |
| `theoretical_short` | 研究模式，不检查券源；必须在 manifest 和结果中显式标记 |

默认 `proxy_margin_secs` 下，Broker 在成交那天判定信用账户开仓资格：成交前 `current_date` 推进到成交日，按 `shortable_by_date`（来自回放槽的逐 `trade_date` 集合）查该日真实 `margin_secs`；逐日数据存在但缺当日集合时 fail-closed，信用开仓一律拒 `margin_secs_data_missing`（不沿用陈旧集合）；仅当回放槽完全没有 margin_secs 域时才回退冻结的决策日 `shortable_codes`。这把 Agent 冻结的决策日快照与 Broker 的同日执行约束隔离开。不在该集合内的 `credit_buy`、`fin_buy` 和 `short` 分别被拒为 `margin_secs_not_collateral`、`margin_secs_not_finable`、`margin_secs_not_shortable`。券商真实担保品池、融资池、券源、逐票费率和担保比例明细接入后需更新本节。
...
- `09:25` 暴露撮合出的开盘价，不含日内最高/最低/成交量，其订单成交于首根连续 bar（09:31），按连续撮合滑点处理。开盘价源仅限开盘时段（≤09:31）的首根 bar：当日首笔行情更晚出现的股票（盘中复牌、迟到首笔）在 09:25 视图中缺席，直到其真实 bar 到达才可见——提前暴露即前视泄漏。
...
- 单一价撮合的两个例外（无区间击穿）：收盘集合竞价订单只对照收盘竞价价（限价可成交则按竞价价清算，否则不成交）；合成 fallback bar（`synthetic=True`）的 high/low 跨越整个交易时段、早于订单存在的时刻，只按参考价成交，杜绝追溯性成交。
- 当日收盘仍未成交的限价单由日终清扫撤单并记录 `order_cancelled`。
- `close` 恒市价；Broker 仍按当日涨跌停、停牌、T+1、账户、保证金、券池和集中度等约束决定是否成交。
- 持仓估值逐 bar 双阶段更新：撮合前按当前 bar `open` 重估已有持仓（资金/保证金准入以订单到达时点的价格计量，而非昨收），撮合后按 bar `close` 重估（Agent 在该 bar 结束后看到的账户状态）。利息计提与强平检查仍保持日终 `mark_to_market` 时点。

分钟数据与 fallback：

- 分钟回放默认使用非空 `intraday_1min.parquet` 的真实分钟 bar。
- 缺失分钟数据的日期/股票，或缺少必要收盘分钟 bar 时，按日线合成 09:30/15:00 两根 bar 作为退化 fallback。合成 bar 携带 `synthetic=True`：09:30 bar 只含开盘价（防前视），15:00 bar 保留全日 high/low 供收盘后观察，但撮合只按参考价（见上文单一价例外）。
...
- 清仓失败的持仓（停牌、涨跌停封板、T+1、缺价）留在账簿并按可见价盯市计入期末权益；`detailed_return.json` 以 `liquidation_complete`、`unliquidated_positions`（含 account/side/数量/市值/受阻原因）和 `remaining_liabilities` 显式报告清算完整性，回测 summary 透出 `liquidation_complete` 与 `unliquidated_position_count`。清算能力不改变主收益口径，也不参与验收判定。
...
| 权益序列 | `equity_curve`：每个交易日收盘后的账户权益时间序列。日收益、Sharpe 和回撤在计算时以初始权益为 Day-0 基准（首日亏损与低于初始权益的峰值都计入），落盘序列本身仍只含日终值 |
