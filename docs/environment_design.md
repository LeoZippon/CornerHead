...
`main` 每个 tick 发出的原语全部是**市价单**，与实盘 QMT 控制器对齐（QMT 无券商侧条件单/止损单）：在某根 bar 决策的单，于其后第 `execution_lag_bars` 根 bar 的开盘价成交，不在决策 bar 内成交，杜绝 bar 内前视。`execution_lag_bars`（默认 2，经 manifest 配置）即决策 bar 到成交 bar 的间隔：`1` 为紧邻下一根，`2` 表示「一根算/报单延迟 + 下一根成交」；如 09:35 决策、09:37 开盘成交。宿主 Broker 据此执行下单、成交、拒单、约束和审计；隔离边界不变（策略只表达意图，不直接改账户）。`ctx.positions` 只反映已成交持仓；在途（已报未成）单经 `ctx.broker.pending(ts_code)` 暴露，与实盘委托查询一致，策略据此对已有在途单的代码跳过重复下单。

分钟回放是默认口径：有非空 `intraday_1min.parquet` 时按真实分钟 bar 推进；缺失分钟数据的日期/股票，或缺少必要收盘分钟 bar 时，按日线合成 09:30/15:00 两根 bar 作为退化 fallback。成交价使用成交 bar 的 `open` 再叠加滑点。盘前两 tick（09:15→09:30 开盘竞价、09:25→09:31 首根连续）为批量集合竞价撮合，不受 `execution_lag_bars` 影响。回放区间最后一个交易日保留为剩余持仓的强制清仓日；临近收盘、其后无第 `execution_lag_bars` 根 bar 的决策无法成交，记 `main_actions_unfilled`。
