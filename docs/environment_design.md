...
- **利息**：融资利息按未还本金、融券费按未还股份×开仓价，均按**自然日 /360** 计入合约（周末/节假日照计），偿还时以现金支付；未付利息同时进入维保比例分母与保证金可用余额扣减项。
- **维持担保比例** = (信用账户现金 + 信用账户证券市值合计) / (融资未还金额 + 融券股份×市价 + 利息费用合计)——**只计信用账户资产，普通账户不作担保**。低于 `maintenance_closeout_ratio`（1.30）触发强制平仓：只清仓信用账户持仓（普通账户不受影响），融券负债随平仓即时归还。**融资负债不会被清仓自动归还**：本金继续计息、由策略显式还款，权益已净额扣除，故不影响收益核算——这是有意的真实行为（欠款不会因平仓消失）。
...
- A 股手数规则（普通 100 股整数倍；科创板 200 股起、1 股递增）、手续费（含最低佣金、过户费）、滑点和印花税。
...
|---|---|
| `ctx.cur_date` / `ctx.cur_time` / `ctx.cur_datetime` | 当前仿真日期、分钟和 Asia/Shanghai ISO 时间戳；统一驱动 Timeview、substep `ready_at`、延迟提交与撮合 |
| `ctx.account` / `ctx.positions` | 只读双账户快照和逐标的持仓快照；持仓行带 `account` |
| `ctx.price(ts_code)` / `ctx.bar(ts_code)` / `ctx.bars` | 当前 tick 可见行情；未来 bar 不可见，09:15 和普通 off-session 通常无实时价格 |
...
| `ctx.asof_dir` | 逐 tick 滚动、节点门控的五个 parquet PIT 域目录：`daily`、`events`、`macro`、`fundamentals`、`intraday_1min` |
...
| `ctx.model_dir` / `ctx.params` | 只读模型产物目录和运行参数；需要跨回测持久的数据应在回测前写入 `models/` |

`ctx.asof_dir` 是逐 tick 滚动的 Timeview，按仿真时钟（`ctx.cur_datetime`，Asia/Shanghai）放行五个 Agent 可直接读取的 parquet 域：`daily`、`events`、`macro`、`fundamentals`、`intraday_1min`。文本库不在 `ctx.asof_dir` 下，宿主侧 `ctx.nl()` 用同一仿真时钟门控公告/新闻可见性。

Timeview 可见性规则：

- 每个域必须等对应本地库刷新任务按仿真时钟完成后才可见；刷新节点来自 `data/contracts.py` 的 `REFRESH_NODES`，并镜像 `configs/tushare_update_schedule.json` 的实际 crontab（`ready_at = start + duration_minutes`）。
- `daily`、`macro`、`intraday_1min`、多数 `events` 跟随 `cn_evening_full`（23:35 启动、约次日 02:05 完成），所以盘中横截面通常只到 D-1。
- 当日实时行情走 `ctx.bars` / `ctx.price`，不进入持久化 Timeview。
- `margin_secs` 在盘前节点后当日可见（约 09:05/09:15）；`fundamentals` 在夜间 PIT 构建后可见（约 03:50）；`cctv_news` / `news` 在盘前文本回填后可经 `ctx.nl()` 检索（约 09:00）。

每个 parquet 域都是普通 parts 目录，可用 `pd.read_parquet(ctx.asof_dir / "<域名>")` 读取当前 tick 已可见的全量 as-of 表。part 0 是冻结研究快照（硬链入，零拷贝），后续 parts 是 write-once 的回放增量，只在仿真时钟跨过该域刷新节点时追加；因此 09:20 到次日 02:05 若无节点完成，视图保持冻结、不会重建。`ctx.asof_version` 在视图滚动时自增，策略可缓存读取结果、仅在版本变化时重算。`ctx.snapshot_dir` 始终是冻结研究基线；`timeview_enabled` 默认开启。`ctx.nl()` 文本同样滚动：冻结研究语料始终可见，新增公告/新闻只在刷新节点完成且 `available_at` 通过门控后可见。
...
| 接口 | 作用 |
|---|---|
| `buy(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 普通账户股票买入（23）；`limit` 为限价单，`valid_bars` 为限价单最多挂单 bar 数（默认 1） |
| `sell(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 普通账户卖出多头可卖（T+1）份额（24） |
| `credit_buy(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 信用账户担保品买入（33），构成信用账户担保资产；当前由 `margin_secs` 近似标的池门控 |
| `credit_sell(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 信用账户担保品卖出（34），所得留在信用账户；融资买入股份必须用 `sell_repay` 卖出还款 |
| `fin_buy(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 融资买入（27）：开仓不动用现金，本金+费用计入融资负债合约、按日计息；受保证金可用余额、标的池与授信额度约束 |
| `short(ts_code, amount, limit=..., valid_bars=1, reason=None)` | 融券卖出（28）；**必须限价申报且申报价不得低于参考最新价（uptick 规则）**，市价 short 拒单 |
| `cover(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 买券还券（29）：平空份额并按最老合约优先偿还融券负债，偿还部分的应计利息即时以现金支付；融券卖出当日不可还券 |
| `sell_repay(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 卖券还款（31）：卖出净所得先息后本偿还融资负债（最老合约优先），余额留作信用账户现金；无融资负债时拒单 |
...
- `amount` 是股数，必须是正整数；主板/创业板等普通 A 股必须为 100 股（1 手）的整数倍，科创板为 200 股起、之后 1 股递增。
...
`main` 发出的交易原语对齐官方 QMT `passorder` / `cancel`。Environment 不引入券商侧条件单或止损单；所有订单都进入当日订单簿，由宿主 Broker 按交易规则、账户约束和行情 bar 撮合。

决策在某根 bar 产生后，订单于其后第 `execution_lag_bars` 根 bar 起进入撮合；默认 2，表示“一根算/报单延迟 + 下一根成交”，如 09:35 决策、09:37 起成交。`execution_lag_bars=1` 表示紧邻下一根 bar 起撮合。已提交但未成交/未撤的订单会占用 `available_cash`、信用保证金可用余额和 `sellable_quantity`；`cash`、持仓 `quantity`、`position()` 只反映已成交真相。

`ctx.broker.pending()` 返回已提交的 submit-lag / 工作订单，可在 `ctx.substep` 内用 `ctx.broker.cancel(order_id, reason=...)` 撤销。提交延迟队列中的订单被撤销时记录 `order_cancelled(pending_stage="submit_lag")`；已进 Broker 订单簿但未成交的订单调用底层 `cancel` 移除；已经在当前激活 bar 成交的市价单不能事后撤销。

`auction_enabled` 默认开启，每个回放日插入 `09:15`、`09:25` 和 `14:57` tick。`09:15` 是盘前信息 tick：集合竞价尚未撮合，`ctx.price` 为 None，可用于筛选与 NL，其订单成交于 09:30 开盘集合竞价。`09:25` 暴露撮合出的开盘价，不含日内最高/最低/成交量，其订单成交于首根连续 bar（09:31），按连续撮合滑点处理。`14:57` 是收盘集合竞价决策 tick，其订单成交于 15:00 bar 的收盘价。

真正的集合竞价成交（09:15→09:30、14:57→15:00）按单一竞价价清算，不计滑点，开盘取 `open`、收盘取 `close`，`price_label="auction"`。

报价与成交：

- **市价单**（默认，对应 prType 14 对手价）：进入 bar 后按 `open` + 滑点成交，单 bar 有效。
- **限价单**（`limit=P`，对应 prType 11 指定价）：自进入 bar 起最多 `valid_bars` 根 bar 有效，不计滑点；买/补在 `open<=P` 时按 open 成交，否则 `low<=P` 时按 P 成交；卖/空在 `open>=P` 时按 open 成交，否则 `high>=P` 时按 P 成交；窗口内未触及则自动撤单并记录 `order_cancelled`。
- `close` 恒市价；Broker 仍按当日涨跌停、停牌、T+1、账户、保证金、券池和集中度等约束决定是否成交。

分钟回放默认使用非空 `intraday_1min.parquet` 的真实分钟 bar。缺失分钟数据的日期/股票，或缺少必要收盘分钟 bar 时，按日线合成 09:30/15:00 两根 bar 作为退化 fallback；`execution_lag_bars` 会按当日 bar 数收敛为 `max(1, min(lag, n-1))`，使两根 bar 退化日仍能在 15:00 成交。09:15、09:25、14:57 的固定竞价/首根连续成交不受 `execution_lag_bars` 影响。

回放区间最后一个交易日保留为剩余持仓强制清仓日；临近收盘且无后续可成交 bar 的决策记录 `main_actions_unfilled`；当日收盘仍挂着的限价单自动撤销。

融券卖出限价规则（实施细则）：融券卖出必须限价申报，市价 short 在 `passorder` 即拒（`slo_sell_requires_limit_price`）。订单首次到达交易所时，若 `limit <` 激活 bar 参考价，按申报被拒并记录 `slo_sell_uptick_rule`；通过检查后正常挂单，此后价格上穿限价属合法成交。典型用法是 `limit=ctx.price(code)` 或更高。


### 3.6 substep、状态写入与计算延迟

正式策略中的实质步骤都应写入 `with ctx.substep(name, budget_minutes=B):`：持仓/在途管理、横截面筛选、模型推理、NL、状态读写、批量下单计划、broker action 和撤单扫描。`B>0`，同一 tick 内 substep 名必须唯一；`B=0` 或超过 `decision_max_sim_minutes` 会在 substep 初始化时被拒。
...
3. **broker action 提交时点**：块内 action 先是提交计划，不会立刻投影到账户/持仓；到达提交时点后再进入 §3.5 的常规撮合规则。

提交时点：

| `B` | broker action 与 state 写入 |
|---|---|
| `0 < B < 1` | 视为本决策分钟内完成；broker action 按当前 tick 提交，再进入常规撮合或竞价规则；state 写入仍按 `ready_at` 合并 |
| `B >= 1` | broker action 等到 `ready_at` 后第一个可报单 tick 才提交；等待期间不是 Broker 委托，不进入 `pending()`，也不能用 `cancel()` 撤销 |

若跨分钟 action ready 后已无后续成交 bar，按常规路径记录 `main_actions_unfilled/no_fill_bar_ahead`，不会静默顺延；若 `ready_at` 落在普通 off-session，则等下一个显式盘前/收盘竞价 tick 或交易分钟 tick。若要改变尚未 ready 的计划，应改写策略状态，让 ready 后的提交条件不再成立。轻量撤单扫描也应放入小预算 substep（如 0.5 分钟），以统一统计耗时和撤单提交时点。
...
- `ctx.state_dir` 只适合小体量跨 tick 状态；高频 substep × 大状态目录会产生 O(state) 拷贝成本。大数据应预先整理进 `models/`，或只在少数 tick 重算后写入 `ctx.state_dir`。
...
最终评估（每个 Fold 的冻结 `test_000` 与 held-out，均 `mode="frozen_eval"`）使用更宽松的防挂死兜底：`backtest_final_eval_max_seconds_per_decision` / `backtest_final_eval_max_seconds_per_trading_day`（默认 900s / 3000s）。该兜底只用于杀掉真正挂死，不作为接受门槛；已在验证阶段满足紧上限的策略应能跑完最终评估，避免结果因机器负载浮动而不可复现。
...
- PIT 输入满足 Timeview 可见性口径：行级 `available_at` 不晚于该域当前刷新节点的可见 cutoff，且该刷新节点已在仿真时钟下完成。
