...
4. 固定 `AT_SNAPSHOT_DIR=/mnt/snapshot`、`AT_AGENT_OUTPUT_DIR=/mnt/agent/output`、`AT_MODEL_DIR=/mnt/agent/models`、`AT_STATE_DIR=/mnt/agent/workspace/.state`（配套 `AT_STATE_STAGING_DIR=/mnt/agent/workspace/.state_staging` 暂存子步骤写入），在 Sandbox 启动一个常驻 `main(ctx)` 进程。
...
交易逻辑全部由 Agent 定义。Environment 在 Sandbox 中启动一个常驻 `main(ctx)` 进程，按回放分钟逐分钟构造市场级 `ctx` 并调用一次 `main(ctx)`（一次覆盖全市场，而非每只股票一次）。`main` 自己决定时序：每分钟管理已有持仓、在选定时点筛选并开新仓，因此可以在任意分钟开/平仓。`ctx` 暴露的字段清单（`broker` 原语、行情、时间/账户、`nl`、`substep`、`asof_dir`/`asof_version`/`snapshot_dir`/`model_dir`/`state_dir` 等）见 `agent_design.md` §5.3；本节定义其中由 Environment 决定的执行、延迟与可见性语义。
...
每个域是一个普通 parquet parts 目录，用 `pd.read_parquet(ctx.asof_dir / "<域名>")` 读出拼接：part 0 是该域的冻结研究快照（硬链入，零拷贝），后续 parts 是 write-once 的回放增量，只在仿真时钟跨过覆盖该域的节点时追加；因此 09:20→次日 02:05 一整段没有节点完成，视图被冻结、零重建。`ctx.asof_version` 在视图滚动时自增，策略可缓存一次读取、仅在版本变化时重算。`ctx.snapshot_dir` 仍是冻结的研究基线。视图开关由 `rolling_asof_enabled` 更名为 `timeview_enabled`（保留弃用别名）。
