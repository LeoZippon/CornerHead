...
回放没有独立的固定总时长上限；总上界随交易日数和每日硬上限自然伸缩。`replay_window=N` 明确表示 N 个执行策略的交易日，并额外读取 1 个退出日；summary 分别报告 `replayed_trade_days` 和 `replayed_exit_days`。Probe 的策略反馈仅给未提交 action 原因和粗粒度策略拒单类别，不给市场/资格拒单、收益、成交或归因；完整 Valid 的结果目录保留完整审计。小窗口调试结果可以帮助估算完整运行成本，但不构成验收门槛。仿真时间预算不能替代墙钟兜底，因为单个 tick 内的死循环不推进仿真时间。
