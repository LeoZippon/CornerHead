...
- **维持担保比例** = (信用账户现金 + 信用账户证券市值合计) / (融资未还金额 + 融券股份×市价 + 利息费用合计)——**只计信用账户资产，普通账户不作担保**。低于 `maintenance_closeout_ratio`（1.30）触发强制平仓：只清仓信用账户持仓（普通账户不受影响），融券负债随平仓即时归还，**强平多头所得现金自动偿还融资负债**（先息后本、最老合约优先，记 `debt_repaid(via="forced_close")`），与真实券商强平流程一致。主动的担保品卖出不自动还款（只有 `sell_repay` 按策略意愿还款）；区间末日强制清仓维持权益净额口径，未还本金不再产生后续计息日。
...
- 限价单：`limit=P`，对应 `prType=11` 指定价；自进入 bar 起最多 `valid_bars` 根 bar 有效（无成交分钟同样计数），不计滑点。
...
- 限价窗口内未击穿则自动撤单并记录 `order_cancelled`。
