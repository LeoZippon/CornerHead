...
**财务 PIT 索引。** 财务表存在多版本记录，Environment 通过 `fundamental_events` 索引确定每条财务事件的可见时间：行级 `available_at` 取公告日当日 **18:00**，公告日优先使用 `f_ann_date`，其次使用 `ann_date`。启用 fundamentals 时，如果该索引缺失或审计失败，snapshot 构造必须直接失败；其他依赖 `available_at` 门控的数据集缺少该列时也必须报错，不能静默放行。
...
**特殊口径修正。** 历史 09:30 分钟条被用作实盘 `stk_auction` 近似输入时，对深圳股票生成校正后的成交量/成交额字段：`00*.SZ` 乘 `0.76`，`30*.SZ` 乘 `0.58`；沪市、北交所和其他时点保持 `1.0`。该规则只用于开盘竞价近似输入；raw 分钟线、日内成交汇总和 15:00 收盘竞价不改写。manifest 至少记录校正规则 ID、适用字段、倍率、适用市场/代码前缀和生成时间。策略代码应读取校正后的派生字段；如果派生字段不存在，不能静默退回未校正字段来模拟开盘竞价。
...
| 网络 | 普通 Fold 默认断网（Docker `--network none`）；元学习默认 Docker `bridge` 网络，经宿主 NAT 访问公网；实验配置可改 Docker 网络模式（`none`/`bridge`/`host`）。元学习默认启用代理别名机制；托管 XRay 有配置时默认启动，命令仍默认直连，Agent 需要代理时只使用当前实验事实中列出的 active `AT_PROXY_*` 别名 |
...
| 包安装 | 普通 Fold 不安装新包，依赖由基础镜像或元学习派生镜像提供；元学习可在开放网络时把实验依赖安装到 `/mnt/agent/workspace` 用户目录 |
...
元学习网络默认使用 Docker `bridge` 网络：容器有独立网络命名空间，通过宿主 NAT 访问公网，不共享宿主的 `localhost`。GitHub/HuggingFace token 只通过环境变量名传递；CLI 会从仓库根目录 `.env` 选择性加载本次允许透传的变量名（如 `GITHUB_TOKEN`、`HF_TOKEN`），Docker 启动后只把实际存在并注入的名称记录为 active，不会打印或写入变量值。常见配置：
...
代理别名机制默认开启，但不会自动启用标准 `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY`。如果仓库根存在本地忽略文件 `.env.xray.json`，或 `.env` 提供了 `META_LEARNING_XRAY_*` 配置，Runner 会在每个元学习 Sandbox 启动前默认拉起宿主侧 XRay 进程，自动选择空闲端口并生成随机 inbound 认证，启动 Docker 时把宿主代理环境映射为容器内 active `AT_PROXY_*`，并在元学习结束后清理进程。`.env` 只用于覆盖默认路径、内联配置或指定非 PATH 中的 XRay 二进制：

```bash
META_LEARNING_XRAY_CONFIG_PATH=/secure/path/xray-meta.json
# 或 META_LEARNING_XRAY_CONFIG_JSON='{"outbounds":[...]}'
# 或 META_LEARNING_XRAY_CONFIG_B64=...
# 可选：META_LEARNING_XRAY_BIN=/usr/local/bin/xray
```

需要关闭代理别名或托管 XRay 时使用；关闭代理别名时托管 XRay 也不会启动：
...
若使用已在宿主运行的 XRay/VPN/代理客户端，也可直接把宿主 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY` 或 `NO_PROXY` 配好；元学习 Docker 启动时只把实际存在的标准变量映射为非标准 active 别名 `AT_PROXY_HTTP`、`AT_PROXY_HTTPS`、`AT_PROXY_ALL`、`AT_PROXY_NO_PROXY`。若 direct GitHub/HuggingFace/PyPI/npm 访问卡顿或失败，Agent 可在单条 shell 命令前临时启用当前实验事实中列出的 active 代理，例如 `HTTPS_PROXY="$AT_PROXY_HTTPS" ALL_PROXY="$AT_PROXY_ALL" hf download ...`。`bridge` 网络下 Environment 优先把 `host.docker.internal` 映射到宿主可达的 `docker0` IPv4，并让托管 XRay 只监听该地址；若 XRay 配置存在但 bridge IP 不可检测，Pipeline fail-fast。`host` 网络下保留 `127.0.0.1` 语义。仓库、manifest、trace 和 system prompt 只记录环境变量名、网络模式、随机端口和使用边界，不记录 token、订阅链接或代理配置正文。GitHub token 由用户用 GitHub/`gh auth` 创建；流程不会自动生成第三方 token。
...
所有工具 trace 都记录当前 `tool_spec` 的 `schema_version` 和 `result_policy`。`shell` 作为工具之一，额外记录 `command_kind`，取值如 `read`、`list`、`search`、`write`、`neutral` 或 `unknown`；该字段用于审计和后续统计，权限判断由 Sandbox、文件系统权限、Docker 网络和阶段策略执行。
...
Shell 不再维护复杂路径或写目标静态解析；读写边界由 Docker `:ro` 挂载、`/mnt/agent` 父目录不可写、`workspace`/`output`/`models` 子目录权限、test 槽不可读权限和普通 Fold `--network none` 执行。Shell 工具层只做阶段开关、`finish_fold` 后写锁检查、参数校验、超时、输出预算、trace 记录和粗粒度 `command_kind` 审计标签。Shell 命令自身失败不转成 ToolError，而是返回非零 `exit_code`、`stderr` 和必要时的 `stdout_path` / `stderr_path`；Tool 层拒绝才返回结构化 `error_type` / `reason` / `retry_hint`。Explore Sub Agent 与主 Agent 共用同一 shell 执行边界，并按只读约定只做数据探查；若误写正式产物，仍由 modification_check、冻结 hash、Docker 只读挂载和产物采集合同兜底。Prompt 与工具 schema 都要求 Agent 不使用 `2>/dev/null` 隐藏错误（命中该模式时 shell 结果附带一条非阻断的 `stderr_suppression_reminder`），stderr 应原样进入 trace。超时清理在容器内执行：每条定时 shell 命令在容器内 `timeout` 下运行（其进程组在超时即被整组杀掉），宿主 `docker exec` 的截止时间只作更长的兜底；容器以 `--init` 启动由 tini 回收孤儿/僵尸，避免被杀的训练子进程残留并占满 `--pids-limit`。
...
`finish_fold` 先把 `output/` 和 `models/` 从宿主侧切到只读，避免校验期间被后台进程竞态修改；若任一检查失败，目录恢复为可写并返回可修复原因。随后按 Pipeline 冻结同口径把关：当前 `output`/`models` hash 必须已有一次成功的完整验证回测（`replay_window` 调试回放不算），最近一次修改检查必须仍匹配当前 hash，并执行轻量合同检查确认当前产物可加载且 `main(ctx)` 存在。成功后，Runner 清理 Sandbox 内 Agent 后台进程，复核当前策略/model hash 未变，保持 `output/` 和 `models/` 只读锁定；Pipeline 再复核验证结果和当前策略/model hash 是否一致。
