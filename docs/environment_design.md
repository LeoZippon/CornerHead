...
- [3. 策略执行、Broker 与回放](#3-策略执行broker-与回放)
  ...
  - [3.2 Broker、账户与模拟交易建模](#32-broker账户与模拟交易建模)
  - [3.3 main(ctx)、逐 tick 执行与数据可见性](#33-mainctx逐-tick-执行与数据可见性)
  - [3.4 订单生命周期、撮合与 substep 延迟](#34-订单生命周期撮合与-substep-延迟)
  - [3.5 回测限时、可观测性与结果摘要](#35-回测限时可观测性与结果摘要)
...
`amount` 是股数，必须是正整数；主板/创业板等普通 A 股必须为 100 股（1 手）的整数倍，科创板为 200 股起、之后 1 股递增。Broker 不做向下取整、超可卖量截断或单票 cap 自动压量，金额/股数超出约束时直接拒单并记录原因。仓位 sizing 由策略显式读取现金、价格和可卖量后自行计算；Broker 不接受 `weight` 下单参数。所有下单/撤单原语都接受可选 `reason=`（默认 `None`）审计注记，Sandbox driver 原样记入 Broker 事件、不影响撮合。下单原语返回可用于撤单的 `order_id`；`pending()` 记录包含 `order_id`、`account`、`op_type`、`submitted_at`、`age_minutes`、`status`，并可带 `pending_stage`（如 `submit_lag`）等字段。所有拒单、撤单、T+1 阻挡、维保警戒、强制平仓和负债合约展期事件必须记录。
...
`op_type` 取官方 opType 码（`optype` 常量）：普通账户 23 股票买入 / 24 股票卖出；信用账户 27 融资买入 / 28 融券卖出 / 29 买券还券 / 31 卖券还款 / 32 直接还款 / 33 担保品买入 / 34 担保品卖出。opType 自身决定账户归属，`passorder` 不需要账户选择参数。**30 直接还券有意不支持**：每个账户内保持单票单侧持仓（`opposite_side_position_open` 拒单），该操作在此约束下结构性不可达，买券还券覆盖其经济需求；未来如需支持须把账户内持仓按 (code, side) 重新建键。`pr_type` 支持 11 指定价（限价，用 price）与 5 最新价 / 14 对手价（市价，回测口径相同）；`order_type` 仅支持 1101（单股/股数），直接还款按官方口径用 1102（金额元）。引擎按决策 + `execution_lag_bars` 把订单 `passorder` 进簿，逐 bar `match_bar` 撮合。
...
### 3.4 订单生命周期、撮合与 substep 延迟

正式策略代码必须在 `ctx.substep` 内调用 `ctx.broker` 原语；子步骤外的下单、平仓或撤单会被 Sandbox driver 拒绝。substep 内 broker action 是按声明预算建模的提交计划，不会立刻投影到账户/持仓；跨分钟动作在 ready 前还不是 Broker 委托，不进入 `pending()`，ready 后才提交并由宿主 Broker 真实约束。`ctx.broker` 的 `cash`、持仓 `quantity` 和 `position()` 只反映已成交真相；`available_cash`、信用账户保证金可用余额和 `sellable_quantity` 会扣除已提交但未成交/未撤的订单占用。同一 substep 内刚记录的 broker action 仍只是本 tick 的本地计划，不会反向修改该 substep 已读取到的账户视图。
