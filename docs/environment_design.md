...
| `auction` | `auction.parquet` | 精确开盘竞价结果（2025-01-16 起；更早为标记代理），按分区实际落地时间逐行滚动 | 开盘竞价研究与 Broker 竞价清算参考；窗口跟随 daily |
...
| 股票筛选 | （SnapshotConfig `screen_*`） | 实验级研究宇宙：剔除 ST（按决策日在市名称）、剔除新股（上市 <N 天）、流通市值带（亿元）、股价带、板块子集 | 只用决策锚点已知信息计算，整个区间冻结不重筛（属性缺失 fail-closed）；同一集合限制 universe/daily/分钟/竞价/事件/财务全部逐股域（决策快照与回放槽一致），显著降低数据量与回测耗时；非 A 股代码的行（行业/概念/指数级 `.TI`/`.DC`/`.KP` 等）是市场级上下文，不参与股票筛选；manifest 记录筛选配置与结果规模 |
...
| `/mnt/snapshots/test/` | 测试或 held-out 回放区间 | 开发容器中不存在、不挂载 | 仅宿主回放读取；原始数据不反馈给 Agent |
| `/mnt/snapshot/` | 当前决策输入视图 | 只读可见 | `main.py` 正式运行时只读 |

`valid` 和宿主侧 `test` 回放槽可以包含回放期行情、事件、宏观、财务、竞价、文本索引+文本库和可选分钟线。正式策略代码不直接选择阶段槽；Valid decision view 镜像到开发容器的 `/mnt/snapshot`，隐藏 Test/Held-out decision view 只挂给一次性 formal 容器，不替换开发容器镜像。
...
- 竞价域（受支持窗口内唯一可能合法为 0 行的域）即使 0 行也保持固定物理 schema，读取器先用 footer 识别空域再应用行过滤；其余域在受支持窗口内不为空，空框仍是零列/null 类型文件。
...
| 文件 | 标准口径与换算 |
|---|---|
| `daily.parquet` | 价格元/股；量与股本为股；金额/市值为元；比例、收益、换手和股息率为小数 |
| `intraday_1min.parquet` | 价格元/股、量为股、金额为元；源数据已是目标口径，无额外倍率 |
| `auction.parquet` | `price/pre_close` 元/股、`vol` 股、`amount` 元；`turnover_rate` 百分数值乘 `0.01`，`float_share` 万股乘 `10000`，`volume_ratio` 保持无量纲倍数 |
...
- Agent 不得把 daily 单位合同外推到其他域；跨域计算必须显式换算成派生列。unknown 与表外字段的使用政策以 Data 文档 §1.2 的权威表述为准。
- 精确源单位见 `docs/units_reference.md`（由注册表生成）。

daily 与 auction 的原始单位和转换规则写入快照清单。研究域保留源单位，策略在跨域计算前必须显式换算（unknown/表外字段政策见 Data 文档 §1.2）。这是策略与审计约束，Broker 无法根据字段血缘自动识别并拒绝订单。
...
开发 Agent 使用长驻 Sandbox；每次 Probe、Valid、Test、Held-out 和合同检查另起一次性 formal 容器。formal 容器断网、只读根，只挂载当前快照、只读 `output`/`models`/可见 state/`asof` 与可写 staging/RPC；不挂载开发 `workspace`、阶段槽或 `/mnt/artifacts`。开发容器本身只挂 train/valid，隐藏 Test replay 从不进入其 mount namespace；隐藏 decision view 也只绑定 formal 容器。开发容器在普通 formal 调用期间暂停并在结束后恢复；最终 Test/Held-out 开始后保持暂停，直至本 Fold/held-out 容器销毁。
...
| 挂载与权限 | 宿主准备快照、运行事实和 Agent 工作目录；开发容器不挂隐藏 Test，formal 容器只挂白名单路径，文件权限仅作纵深防御 |
...
| Python | Docker 镜像使用 Python 3.11；正式策略进程默认固定 `PYTHONHASHSEED=0`，使未排序容器的跨进程迭代可复现；准确依赖、工具和版本由运行环境事实记录 |
| 本机环境 | 本机脚本、测试和 cron 使用 `~/miniconda3/envs/quant`，与 Docker Python 独立 |
| 包安装 | 普通 Fold 不安装新包，依赖由基础镜像或元学习派生镜像提供；元学习可在开放网络时临时试装依赖，需继承给后续 Fold 的稳定依赖必须声明为派生镜像请求。基础镜像预烘焙 C/C++/Fortran 与匹配 torch 轮子 CUDA 版本的 CUDA 编译工具链（nvcc，`TORCH_CUDA_ARCH_LIST` 固定为宿主 GPU 架构），使派生镜像可直接源码编译 torch_scatter/pyg_lib 等 CUDA 扩展包 |
...
| 写入面 | 开发阶段仅 `/mnt/agent/workspace`、未锁定的 `/mnt/agent/output` 和未锁定的 `/mnt/agent/models`；formal 策略仅写隔离 staging/RPC |
...
/mnt/artifacts/
  ...
  results/
    <phase>_<idx>/
      ...
      style_analysis.json         # 每次完整回放固定写出
...
- `/mnt/artifacts/runtime_env.json` 是 Sandbox 运行环境契约，记录 Python 包、CLI 工具、网络和包安装策略；Docker 模式下由宿主在会话启动前对本次实际使用的镜像做一次离线探针生成（包清单为发行版名→版本的完整清单，派生镜像的新增包自动出现），不再维护静态清单；Agent 可读，正式策略代码不得硬编码读取。
...
- 系统 Prompt 的“当前实验事实”只呈现身份、可见性、窗口、预算、产物合同、Broker/replay 和精简工具能力；完整路径、数据摘要、单位合同与包/CLI 清单保留在三个运行 JSON，不重复内联。
...
| `web_fetch` | 元学习读取公开网页 | 仅元学习可用；宿主侧只读 GET。默认直连：解析并校验为公网地址后把 TCP 连接固定到该已校验地址（TLS 主机名校验保留），防止校验与连接之间的 DNS 重绑定；`use_proxy=true` 才允许使用 active 代理（代理路径由代理自行解析）。只支持 http/https 文本或 HTML，跨 host redirect 不自动跟随，结果写 trace |
| `modification_check` | 校验正式 `output` 修改量、`models` 格式/大小和父产物 hash | 无业务参数；不检查 `workspace` 或结果目录 |
| `backtest` | 执行 `output/main.py` 并回放交易；Agent 可传的业务参数只有 `replay_window`（前 N 个策略交易日另加 1 个退出日；Probe 不执行 NL 内容生成，只返回匿名成本/生命周期、未提交 action 原因和粗粒度策略拒单类别，不产生收益、成交或归因文件） | 消费并校验当前 snapshot；Probe 不暴露市场/资格拒单，完整 Valid 保留完整审计；每次调用创建唯一结果目录 |
...
- 系统 Prompt 只保留跨工具使用原则和关键边界；工具名、参数语义、输出预算与分页方式由 native schema 和字段 description 单源提供，失败原因与重试建议由 `ToolError.error_type/reason/retry_hint` 返回。
...
- 主要按估算 Prompt token 触发，消息数只作高位安全上限；裁剪或压缩会重置模型前缀缓存。确定性裁剪命中 150 条上限时批量留出 30 条消息余量，而非逐轮只丢新增溢出，使一次不可避免的前缀重写可由后续多轮摊销；总上限与语义压缩条件不变。
- 主对话按 prompt/completion/reasoning 以及缓存命中/未命中累计 token，并写入 session 摘要的 `token_usage`（含 `cache_hit_ratio`），用于权衡裁剪/压缩强度。


**Shell 执行边界**

- Shell 不尝试静态解析复杂 Bash、路径或写目标。硬边界由挂载白名单、只读挂载、父目录权限、受控写入目录和普通 Fold 断网共同执行；隐藏 Test 不挂载到开发容器。
...
- Docker 挂载白名单是主要隔离层，文件权限仅作纵深防御。Agent 可见工具结果只返回 Sandbox/相对路径；宿主路径仅留在 host manifest 等未挂载审计面。
...
result = nl(
    ts_code,
    prompt="...",
    event_filter={"patterns": ["处罚|诉讼|退市"], "lookback_days": 30},
...
2. 单股请求若声明 `event_filter={patterns, lookback_days}`，先在代码/公司名关联的候选材料中应用 PIT 可见性，再限制到 `[as_of-lookback_days, as_of]` 并匹配标题或正文。零匹配直接返回 `status="ok", state="no_matching_evidence", content=""`，不执行 provider 或模型内检索。
3. 对同一代码、Prompt、参数和公司上下文，宿主按当前匹配范围的内容 revision 复用成功或无证据结果；事件进入或移出滚动窗口都会改变 revision。未声明过滤器的单股自由分析保守地按全部候选证据校验，并最多复用到当前仿真日期结束。全市场请求不复用。
4. 未命中复用时，只检索当前时点可见且位于声明窗口内的文本，并启动有界分析会话；单股请求附加公司上下文与权威 `decision_as_of`。`event_filter + enum` 直接复用门控已匹配的最多 5 条证据并禁用模型工具调用，只做 1 次规范标签回答，不再让模型重复规划或扫描同一候选范围。
5. 返回内容、证据引用、工具摘要、复用状态和错误状态，并写入本次回测审计目录。

NL 默认回答自由文本；窄标签任务可声明结构化输出：

- 证券代码可选；传入时标题与正文均严格限定到 PIT 索引按代码或公司名关联的候选材料；不传代码时才执行事件、主题、行业、宏观或市场级检索。
- `response_format={"type":"enum","values":[...]}` 单独使用时最多 1 个检索轮次和 1 个强制最终回答，并保留配置的推理模式；与 `event_filter` 同用时直接采用门控证据、关闭隐藏推理并仅调用 provider 1 次。最多传入 5 条、每条 1,000 字符证据片段，输出预算 512 token。通用自由文本同样保留配置的推理模式。
- enum 成功结果的 `content` 是一个规范允许值；宿主取回答中首个独立允许值，避免后文提及其他标签造成子串误判。原始回答仍保留在 provider 审计日志。
- 未声明 `response_format` 时保持通用的 3 个检索轮次、自由文本合同，策略自行解析并处理失败。
- 所有模式都返回状态、范围、正文、工具摘要、证据和错误等字段。
...
事件过滤器由策略显式声明，不从模型搜索词猜测，因此有效期、窗口和缓存失效语义都可审计。`patterns` 只缩小已经由代码/公司名关联且 PIT 可见的单股候选范围，绝不扩大可见性。失败结果不缓存；`nl_requests.jsonl` 每次调用均保留记录，复用或无证据调用不伪造新的检索、证据或 provider 日志。
...
NL 临时通信目录由宿主创建并锁定：请求侧只追加，响应侧只由宿主写入。完整 Valid 的明细进入结果目录；Probe 返回 `withheld_probe` 并不生成 NL 内容，Test/Held-out 明细只留宿主审计面。回测后清理临时 RPC 文件。
...
- 网页抓取默认直连，只有显式请求才使用当前代理；仅支持公开 HTTP(S) 文本的只读 GET 与有界落盘。直连时主机名解析结果先做公网校验，再把套接字固定到该已校验地址（防 DNS 重绑定 TOCTOU）；代理请求由代理自行解析目标。
...
- Test/Held-out 的 LLM/NL 明细不反馈给 Agent；compact Test 指标投影不包含这些明细。
...
1. 校验策略、模型和决策输入身份。正式回测要求当前修改检查通过；检查缺失或过期时，工具自动刷新检查，而不是沿用旧结果。验证回放按 snapshot_id 身份比对绑定输入（记录在案的成本决策：热路径不做整槽 hash）；冻结评估（frozen_eval）另对绑定的决策输入与回放槽执行整槽内容 hash 复验，一次性最终评估获得最强完整性检查。
2. 创建唯一结果目录，隔离本次正式输出。
3. 暂停开发容器并启动一次性 formal 策略进程；策略只能访问当前快照、只读代码/模型/可见 state 和可写暂存区。
...
| `frozen_eval` | 仅 formal 容器可见的测试或 held-out `/mnt/snapshot` | 宿主 Test/Held-out replay（不挂载给开发容器） | `results/test_<idx>/` 或 `heldout_<idx>/` | 不直接反馈给 Fold Agent；已完成 Fold 的 compact Test 指标可投影给后续 Meta，Held-out 永不投影 |
...
- 佣金、最低佣金、过户费、印花税和市价滑点由统一成本模型执行；固定滑点不随订单规模和买卖价差变化，是明确的研究近似。滑点是流动性假设而非交易所行为：成交价在加滑点后**钳制在当日涨跌停区间内**，不会打印出交易所不可能存在的价格。
...
- **账户间划转**：策略在 09:14 前提交当日申请，Environment 于 09:14 统一确认。冻结的融券所得不可划出；信用账户有负债时，划出后维保比例必须不低于配置的提取线。注意 substep 延迟的 tick 粒度效应：`ready_at` 按 tick 网格释放，若跨分钟 transfer 的就绪落点已越过 09:14（例如 09:0x 提交、下一 off-session tick 在 09:14 之后），按 `transfer_after_preopen_cutoff` 拒绝——需要当日划转时应在更早的 tick 用小预算 substep 提交。
...
- 非决策分钟上 Timeview、状态合并、订单激活和 Broker 生命周期仍继续推进。
- 当前仓库只实现回放；同一策略接口只是未来实盘目标，不代表实盘执行器已存在。

执行节奏：

- 所有正式回放共用固定 A 股交易分钟时钟：09:30–11:30、13:00–15:00 逐分钟推进，另有 09:15/09:25 竞价 tick；行情只是附着在时钟上的可选市场事件，数据是否存在不得增删盘中时间。
- 普通盘中分钟上 `main(ctx)` 的调用间距可配置；跳过调用的分钟仍推进 PIT 时间、Timeview、状态合并、substep `ready_at`、订单激活和在途年龄，只有存在行情事件时 Broker 才撮合。
- 时段外按配置间隔继续调用 `main(ctx)`，只用于研究、状态和计划维护。
- 普通 off-session tick 不提交交易所订单；`transfer` 是盘前资金划转申请，不是交易所委托。
- 固定交易时钟内均可在 `ctx.substep` 中报单、平仓或撤单；当前分钟没有行情时 `ctx.price`/`ctx.bar` 为空，订单激活后留在 Broker 等待下一市场事件。普通 off-session 与竞价结果研究 tick 不可报单。当前时间表见 [参数参考](parameters_reference.md#3-回放执行与预算)。
...
|---|---|
| `ctx.cur_datetime` | 权威仿真时间戳，Asia/Shanghai ISO-8601 字符串；直接使用，需要对象时用 `datetime.fromisoformat()`；驱动 Timeview、substep `ready_at`、延迟提交与撮合 |
...
| `ctx.account` | 当前 tick 入口的只读双账户快照：`stock`、`credit`、`total_assets`、`risk_limits` |
| `ctx.positions` | 当前 tick 入口的只读逐标的持仓快照列表；每行带 `account`，用于区分普通账户和信用账户持仓 |
| `ctx.price(ts_code)` | 当前 tick 该股票可见价格；未来价格不可见，09:15、普通 off-session 以及没有市场事件的固定盘中分钟为 `None` |
| `ctx.bar(ts_code)` | 当前 tick 该股票可见市场事件 bar；该分钟没有事件或该股票无行时为 `None` |
...
| `ctx.nl(ts_code?, prompt=..., event_filter?=..., response_format?=...)` | 决策阶段 NL 工具；必须在 `ctx.substep` 内。单股可声明滚动 PIT 事件过滤器与 enum 输出合同；未声明时保持通用自由文本模式 |
| `ctx.asof_dir` | 路径字符串；逐 tick 滚动、节点门控的 PIT 视图：`daily`、`events`、`macro`、`fundamentals`、`intraday_1min`、`text_index` 和 `text_library` |
| `ctx.asof_version` | Timeview 任一域真正滚动时变化的全局版本串；分钟域也会推进它，重型单域特征应使用更窄的策略缓存 key |
| `ctx.snapshot_dir` | 路径字符串；冻结研究基线快照，不随回放 tick 滚动 |
| `ctx.state_dir` | 宿主管理的跨 tick 状态目录；只能在 `ctx.substep` 内访问，首次访问才复制可见状态，写入暂存至 `ready_at` 才可见 |
| `ctx.model_dir` | 路径字符串；只读模型产物目录；需要跨回测持久的数据应在回测前写入 `models/` |

上述路径字符串（含 `ctx.state_dir`）使用 `/` 拼接前先转为 `Path(str(ctx.asof_dir))` 等 `Path` 对象。`ctx.asof_dir` 是逐 tick 滚动的 PIT 视图，按 Asia/Shanghai 仿真时钟放行日频、事件、宏观、财务、分钟和文本数据。宿主 NL 使用同一时钟门控公告与新闻。

`ctx.account`、`ctx.positions`、`ctx.broker.stock` 和 `ctx.broker.credit` 在一次 `main(ctx)` 内保持 tick 入口快照；同 tick 新 action 只进入 action 队列（已提交的轻量单也进入 `pending()`），不回写账户。批量下单应从入口快照取一次总预算，在本地随每笔计划递减，并为费用和滑点留缓冲。

Timeview 可见性规则：

- 每个域的可见截止只取当前仿真时钟下已经完成的最新落库刷新节点。节点必须对应 Data 文档中的真实落库任务；只读审计任务不能推进可见性。
- 常见时点：多数日频、分钟历史、宏观、批量事件和批量文本跟随 `cn_evening_full`（历史缺实测完成账本时保守按 03:05），盘中通常只到 D-1；`fundamentals` 约 03:50；`margin_secs` 约 09:05/09:15；`cctv_news` / `news` 约 09:00；竞价按分区实际落地时间。
- 当日实时行情走 `ctx.bars` / `ctx.price`，不进入持久化 Timeview。
- 历史晚间节点目前只排除周末，尚无逐次完成账本和法定休市日映射；法定休市的工作日上，实际任务会因终点解析到最近开市日而整体跳过，模型却按已完成放行——对当日更新的全球/宏观行这是**乐观**方向的近似（提前 1–2 天可见），待接入 SSE 日历修正。冻结首片按源端 `available_at` 生成，尚未叠加统一的本地落库门禁（保守方向）。两项均不宣称精确复刻历史落库。

滚动视图采用只追加分片：

- 初始分片是冻结研究快照；合法的零行、非 null Arrow schema 文件也直接零拷贝作为初始分片，旧的零列/null-typed 文件仍保持空目录；后续分片只在仿真时钟跨过对应刷新节点时追加。
- Parts 域由 Pandas 直接读取目录；DuckDB 必须读取 `目录/*.parquet`。当前没有 part 时表示该域暂无可见行。
- 分钟回放按交易日读取；Timeview 只暂存尚未放行的逐日分片，放行后立即释放，不保留整段验证区间。
- 没有节点完成时，视图保持不变，不因 tick 推进反复重建。
- 同一 run 内对相同快照/回放输入的多次回测，分片由宿主侧暂存目录硬链接复用（不重新切片编码）；可见性仍只在滚动时刻产生，复用前校验行数，不匹配即失败。
- 文本正文只保留当前可见索引引用的分片。
- 任一域真正变化时全局版本号递增；它适合判断整个视图是否变化，不等于单域版本。策略应按实际依赖选择固定研究时点、日期或自定义 key，避免分钟域更新触发无关大表重算。
- 冻结研究基线始终不变；滚动视图默认开启。
...
| `stock` | tick 入口的普通账户视图 dict：`cash`、`available_cash`、`total_assets`、`market_value`；`cash` 是已成交真相，`available_cash` 扣已提交未成交买单冻结 |
| `credit` | tick 入口的信用账户视图 dict：`cash`、`available_cash`、维保比例、保证金可用余额、融资/融券负债、应计利息、额度、利率；可用现金/保证金扣融券冻结所得和已提交未成交订单占用 |
| `account` | tick 入口的双账户快照 `{stock, credit, total_assets, risk_limits}` |
| `positions` | tick 入口的逐标的持仓快照列表（每行带 `account`；数量、可卖/可平数量、方向、成本和市值等） |
| `debt_contracts(ts_code=None)` | 未了结融资/融券负债合约明细（未还金额/量、开仓日、年利率、已计未付利息） |

接口通用规则：

- `amount` 是股数，必须是正整数；买入方向沪深主板/创业板必须为 100 股（1 手）的整数倍，科创板为 200 股起、之后 1 股递增，北交所为 100 股起、之后 1 股递增。卖出/还券方向可整手申报，或按交易所零股规则把送转等产生的不足一手零股一次性全部带上（整手数 + 全部零股，或恰为可卖全量；科创板/北交所低于最低申报量的持仓只能整份卖出）。
- Broker 不做向下取整、超可卖量截断或单票 cap 自动压量；金额/股数超出约束时直接拒单并记录原因。
- 仓位 sizing 由策略显式读取现金、价格和可卖量后自行计算；批量 action 需本地维护剩余预算并预留费用/滑点，不能依赖同 tick 账户快照随调用变化；Broker 不接受 `weight` 下单参数。
...
- 同一 Bar 按订单簿 FIFO 逐单校验和撮合；已成交或拒绝的前序单立即释放占用，只有更早且仍挂单的订单继续冻结，后序单不能反向占用当前单资源。
...
- 每个回放日固定插入盘前、开盘撮合和收盘竞价 tick。
- `09:15`、`09:25`、`14:57` 是固定市场语义，不是 `ExperimentConfig` 参数；所有正式回放使用同一日内合同。
- `09:15` 是盘前信息 tick：集合竞价尚未撮合，`ctx.price` 为 None，可用于筛选与 NL，其订单成交于 09:30 开盘集合竞价。
- `09:25` 是盲提交 tick，不能从未来09:30/09:31行情反推开盘价；此时提交的订单于09:31进入 Broker，完整分钟行情下从首根连续 Bar 撮合，缺行情时继续挂起。
- `stk_auction` 完整结果按盘前任务的实际落地时间可见（通常09:27–09:29）。09:30前的结果 tick 只用于研究，不能报单；09:30后的到达唤醒对应固定分钟 tick。Broker 可用隐藏清算真值执行09:15委托，但不得提前暴露给 Agent。
- 09:25 起不能撤销此前开盘竞价委托；竞价结果/off-session tick 不能下单或撤单。14:57 收盘竞价委托进入不可撤阶段。
- `14:57` 是固定收盘集合竞价决策 tick，其订单成交于 15:00 市场事件的收盘价；有无14:57分钟行情都不影响该 tick 存在。缺行情时 `ctx.price` 为空，不伪造未来收盘价。
...
- 启用盘后固定价格交易后，在固定15:00收盘事件后插入盘后定价 tick：`ctx.bars` 为当日收盘事件（已确认收盘价可见）。
...
- 普通市价单在固定时钟的激活分钟进入 Broker；该分钟有证券 Bar 时按开盘价加方向性滑点成交，没有时继续挂单，直到当日下一市场事件。该事件可以是日线合成的15:00收盘事件。挂单期间继续占用估算资金或保证金。
...
- 回放时间结构唯一且固定，不存在“分钟引擎/日线引擎”两条路径；`replay_granularity` 始终为 `minute`。
- `include_intraday`/`replay_include_minutes` 只控制分钟行情是否进入决策快照和回放槽，不控制 Environment 时钟。
- 回放槽存在非空 `intraday_1min.parquet` 时，将真实分钟 Bar 作为对应固定分钟的市场事件。
- 单文件按 `trade_date` 谓词逐日读取，当前日运行时只预取下一交易日；不预建容器，也不跨 Fold 预取。
- Broker 只生成撮合需要的 OHLCVA 视图；Timeview 沿用原始列和原刷新节点，因此 Agent 可见数据合同不变。
- 某日或某证券缺分钟数据，或缺少必要开/收盘 Bar 时，按日线补充09:30开盘和15:00收盘市场事件；中间分钟不伪造 Bar。
- 开盘合成 Bar 只含开盘价以防前视；收盘合成 Bar 可供收盘后观察全日范围，但撮合只使用参考价，不使用全日 high/low。
- `execution_lag_bars` 按固定242个交易分钟计算（兼容键名保留），不会因行情缺行而缩短；订单在激活分钟进入 Broker，随后等待下一市场事件。
- `09:15`、`09:25`、`14:57` 的固定竞价/首根连续成交不受 `execution_lag_bars` 影响。

收尾规则：

- 回放区间最后一个交易日保留为剩余持仓强制清仓日。末日强制清仓与维保强平使用同一负债结清规则：卖出融资持仓即释放融资股份，并以卖出所得按最老合约优先、先息后本偿还融资（Broker 事件分别标注 `forced_close` / `mandatory_exit`）。
- 清仓失败的持仓（停牌、涨跌停封板、T+1、缺价）留在账簿，并按可见价格计入期末权益；结果同时记录未清仓仓位、剩余负债和受阻原因。主收益使用期末盯市净值；清算完整性作为独立诊断，不参与验收判定。`liquidation_complete` 为真要求同时满足：无未清仓仓位，且残余融资/融券负债为零。
- 临近收盘且无后续订单激活分钟的决策记录 `main_actions_unfilled`。
- 当日收盘仍挂着的限价单自动撤销。
...
2. **`ctx.state_dir` 写可见性**：首次访问时才把可见状态复制为块内暂存视图；写入在 `ready_at = 决策 tick + B` 后才并入可见目录，未访问 state 的 broker-only block 不创建副本。
...
| `B >= 1` | 动作等到就绪时刻；交易所订单仅当生成、就绪和释放时点均处于可申报窗口才提交，否则记录未提交或未成交，不自动顺延。`transfer` / `direct_repay` 是现金操作，不受该窗口匹配约束：transfer 到期后进入盘前划转队列（仍受当日 09:14 截止），direct_repay 到期后在可报单 tick 即时结算、无需后续成交 Bar（到期落在盘外仍记录未提交） |
...
- state 合并逐级使用不跟随符号链接的目录句柄，最终文件原子替换；注册后换父目录、FIFO 或目标链接会被拒绝，单个暂存文件上限64 MiB。

- substep 首次访问 `ctx.state_dir` 时，当前可见状态复制到暂存视图；从不访问则不创建暂存副本。
...
- 状态目录只适合小体量、跨 tick 可变状态；高频访问 state 的 substep 会反复复制整个状态目录，纯 Broker block 不会。
...
| 类别 | 字段/内容 |
|---|---|
| 收益与风险 | 总收益、long/short 收益、年化收益、Sharpe、最大回撤、胜率、turnover（年化统一按 244 个 A 股交易日） |
...
- 多窗口完成后，Pipeline 在拼接后的日序列上重跑回归，并按有效天数合并风格暴露；`valid` 前缀的汇总只取账本选定的验证窗口——同一 Fold 的多次验证窗口在相同日期上对应不同策略版本，混合会拼出与任何真实回放都不符的嵌合序列。
...
- runtime env 路径，以及普通 Fold 或元学习需要的实验参数摘要；Meta 同样收到固定竞价合同和执行延迟等实际回放信息，不需从历史 Taste 推测。
- 修改约束和 deadline。
- 关键结果目录和状态摘要。
- 元学习的 development 历史、账本投影、记忆和 Taste 只引用 Sandbox 内路径，不暴露宿主绝对路径；历史 Fold 只允许白名单 Test 指标，不允许 Test 排程、snapshot 身份、结果路径、Trace、订单或 NL/Broker 明细。

宿主收集目录额外保留 `host_run_manifest.json`，用于完整审计测试/held-out 调度、测试 snapshot 和 frozen evaluation 结果；该文件不在 Sandbox 中挂载。

`agent_trace.jsonl` 是轻量事件流，包含 Agent 可见的工具调用（含 shell）、Valid 回测、Broker、LLM、context compact、NL、错误和锁定事件。事件共享 `experiment_id/fold_id/run_id/conversation_id/call_id/parent_call_id`；Agent 可见结果和 Step attachment 只记录容器内或相对路径，不记录宿主绝对路径。


**读取权限**

- Fold Agent 在策略探索阶段只能读取当前 Fold 的验证结果。
- Test/Held-out 原始数据、结果目录、日志、NL 明细和 Broker 事件不反馈给任何开发 Agent。后续 Meta Agent 只接收已完成 Fold 的 compact Test 指标白名单；Held-out 结果从不投影。
- 宿主可读完整审计目录。
