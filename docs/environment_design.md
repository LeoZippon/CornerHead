...
每次实验同时运行**两个独立账户**（如同真实投资者在同一券商的两户）：`stock` 普通账户（纯现金、long-only）与 `credit` 信用账户（担保品买卖 + 融资融券）。两账户现金、持仓与 T+1 各自独立、互不担保；官方 opType 自身决定账户归属（23/24 普通，27–34 信用），`passorder` 无需额外账户选择器。同一票允许普通账户做多 + 信用账户融券做空（对冲）；每个账户内保持单票单侧。现金可经 `transfer` 在两账户间划转（银证转账式 sim 扩展，实盘为人工操作）：融券冻结所得不可划出，信用账户有负债时划出后维保比例必须 ≥ `maintenance_withdraw_ratio`（提取线由此被强制执行）。

`main(ctx)` 内可用的 `ctx.broker` 接口（下单原语均以 `ts_code` 为第一参数）：

| 接口 | 作用 |
|---|---|
| `buy(ts_code, amount\|weight, limit=None, valid_bars=1, reason=None)` | 普通账户股票买入（23）；`limit` 为限价单 |
| `sell(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 普通账户卖出多头可卖（T+1）份额（24） |
| `credit_buy(ts_code, amount\|weight, limit=None, valid_bars=1, reason=None)` | 信用账户担保品买入（33），构成信用账户担保资产 |
| `credit_sell(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 信用账户担保品卖出（34），所得留在信用账户 |
| `fin_buy(ts_code, amount\|weight, limit=None, valid_bars=1, reason=None)` | 融资买入（27）：开仓不动用现金，本金+佣金计入融资负债合约、按日计息；受保证金可用余额、标的池与授信额度约束 |
| `short(ts_code, amount\|weight, limit=..., valid_bars=1, reason=None)` | 融券卖出（28）；**必须限价申报且申报价不得低于参考最新价（uptick 规则）**，市价 short 拒单 |
| `cover(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 买券还券（29）：平空份额并按最老合约优先偿还融券负债，偿还部分的应计利息即时以现金支付 |
| `sell_repay(ts_code, amount, limit=None, valid_bars=1, reason=None)` | 卖券还款（31）：卖出净所得先息后本偿还融资负债（最老合约优先），余额留作信用账户现金；无融资负债时拒单 |
| `direct_repay(amount, reason=None)` | 直接还款（32）：从信用账户现金偿还融资负债（先息后本），金额截断到可用现金与负债余额；提交 tick 即时结算、无撮合 |
| `transfer(amount, from_account, to_account, reason=None)` | 两账户间现金划转；金额超可用或触及提取线时拒单（`insufficient_cash` / `credit_withdraw_blocked_by_maintenance`）；提交 tick 即时结算 |
...
| `pending(ts_code=None)` | 在途/延迟提交单（记录含 `account`）；有参返回该股在途单，无参返回全量 |
| `stock` | 普通账户视图 dict：`cash`、`available_cash`（=cash）、`total_assets`、`market_value`；每 tick 反映已成交结果，未成交计划不改变它 |
| `credit` | 信用账户视图 dict：`cash`、`available_cash`（扣融券冻结所得）、维保比例、保证金可用余额、融资/融券负债、应计利息、额度、利率 |
...
`amount` 是股数（按 100 股，即 1 手，向下对齐），`weight` 是**下单目标账户**初始资金（`stock_initial_cash` / `credit_initial_cash`）的名义比例。所有下单/撤单原语都接受可选 `reason=`（默认 `None`）审计注记，Sandbox driver 原样记入 Broker 事件、不影响撮合。下单原语返回可用于撤单的 `order_id`；`pending()` 记录包含 `order_id`、`account`、`op_type`、`submitted_at`、`age_minutes`、`status`，并可带 `pending_stage`（如 `substep_delay` / `submit_lag`）等字段。`substep_delay` 阶段的 `age_minutes` 从动作生成 tick 起算；ready 后进入 `submit_lag` 阶段则从实际提交 tick 起算。所有拒单、撤单、部分成交、T+1 阻挡和强制平仓事件必须记录。
...
| 项目 | 默认口径 |
|---|---|
| 账户结构 | 固定双账户：`stock` 普通账户（long-only 现金）+ `credit` 信用账户（担保品买卖 + 融资融券），互不担保 |
...
- **账户间划转** `transfer(amount, from, to)`：银证转账式即时现金划转（sim 扩展，实盘为人工操作）。融券冻结所得不可划出（available_cash 门控）；信用账户有负债时，划出后维保比例必须 ≥ `maintenance_withdraw_ratio`（3.00），否则拒 `credit_withdraw_blocked_by_maintenance`。
- **普通账户**：无负债/保证金/维保概念，`available_cash == cash`，只执行现金、T+1、手数、涨跌停、停牌与集中度约束；集中度类约束（`max_total_holdings` 按跨账户去重代码数、`max_single_name_weight` 按跨账户合并名义 / 合并初始权益）在两账户之上统一执行。
