...
开发 Agent 使用长驻 Sandbox；每次 Probe、Valid、Test、Held-out 和合同检查另起一次性 formal 容器。formal 容器断网、只读根，只挂载当前快照、只读 `output`/`models`/可见 state/`asof` 与可写 staging/RPC；不挂载开发 `workspace`、阶段槽或 `/mnt/artifacts`。开发容器从校验到结果发布全程暂停，结束后销毁 formal 容器，避免共享 inode 被后台进程并发修改。
