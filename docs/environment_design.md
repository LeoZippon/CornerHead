...
- [3. 策略执行、Broker 与回放](#3-策略执行broker-与回放)
  ...
  - [3.3 ctx.broker 接口与 QMT 映射](#33-ctxbroker-接口与-qmt-映射)
  - [3.4 main(ctx) 执行模型与 Timeview 可见性](#34-mainctx-执行模型与-timeview-可见性)
...
### 3.4 main(ctx) 执行模型与 Timeview 可见性
...
`main` 每个 tick 都可核对持仓、在途订单和滚动数据，并自行决定筛选、推理、下单时点。`ctx` 字段清单（`broker`、行情、时间/账户、`nl`、`substep`、`asof_dir`/`asof_version`/`snapshot_dir`/`model_dir`/`state_dir` 等）见 `agent_design.md` §3.2；本节只定义 Environment 决定的执行和可见性语义。
