...
4. 未命中复用时，只检索当前时点可见且位于声明窗口内的文本，并启动有界分析会话；单股请求附加公司上下文与权威 `decision_as_of`。
...
- `response_format={"type":"enum","values":[...]}` 把任务限制为 1 个检索轮次和 1 个强制最终回答；provider 最多返回 5 条、每条 1,000 字符证据片段，输出预算 128 token。
