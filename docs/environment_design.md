...
| `backtest` | 执行 `output/main.py` 并回放交易；Agent 可传的业务参数只有 `replay_window`（前 N 个策略交易日另加 1 个退出日；探针不执行 NL 内容生成，只返回匿名 substep 成本和订单生命周期统计，不产生收益、成交或归因文件） | 消费并校验当前 snapshot；每次调用创建唯一结果目录 |
...
回放没有独立的固定总时长上限；总上界随交易日数和每日硬上限自然伸缩。`replay_window=N` 明确表示 N 个执行策略的交易日，并额外读取 1 个退出日；summary 分别报告 `replayed_trade_days` 和 `replayed_exit_days`。小窗口调试结果可以帮助估算完整运行成本，但不构成验收门槛。仿真时间预算不能替代墙钟兜底，因为单个 tick 内的死循环不推进仿真时间。
