...
- 评估后延后为后续工作（不在本分支落地）：ops 脚本生产主机/密钥参数化脱敏与部署文档改写（需运维先行 provisioning）、SIGCHLD 本地回收、快照 staging 目录事务化发布、Timeview schema 漂移硬拒绝与文本索引严格映射、ExperimentConfig/SnapshotConfig 严格标量校验、engine 同分钟 tick 归一（与被排除的回退删除纠缠）。
- 运维附带处置：发现主工作树被另一 Agent 切至其分支（夜间 cron 将运行归档迁移前的 share_float 代码），已恢复至 fix/share-float-incremental-archive；raw 湖自 7-23 03:43 起 dirty（cn_nightly_pit_event_build 连续两晚被 SIGKILL，疑夜间内存压力，swap 满），按世代恢复合同以完全一致的 (job, 区间, command_hash) 精确重跑成功——湖回到 committed（新世代 bde41f8e），当晚 cron 链解封，缺失的 20260722 margin 分区落在晚间 30 天滚动窗口内可自愈。
- Validation：整合后全量 tests/unit 956 passed + 49 subtests（170.8s）；`git diff --check` clean；两个来源分支与主工作树（fix 分支、干净）均未改动。

2026-07-23 全库精简重构（refactor/repo-simplification 分支，不并主干、待二次审计）

- 全库通读（6 份活文档亲读 + 11 路子代理证据报告 + 决策史地雷清单）后按收益/风险分四波落地；全程尊重既往"不做"决策（不模板化 download/audit、不合并 Runner/Explore/NL 工具循环、不拆 app.js/reporting/search/driver、不改 driver/broker_core 镜像烘焙文件、不新增缓存层）。
- 漏洞修复：①Agent 可见 manifest 的 experiment_parameters 投影补齐 test_* 剥离（原 fail-open 泄漏面，新增投影回归测试）；②.raw_generation.json 三处读取器口径分裂收敛到 contracts.py 单一严格 committed 合同，残缺戳 fail-closed（新增负路径测试）；③QMT 桥同包订单 remark 冲突在校验期整包拒绝（保护幂等墙，新增三例回归）；④Step 门控只在成功完整验证后触发（原实现失败回测也挂起，与 pipeline 文档矛盾）；⑤关闭语义压缩时确定性裁剪成为真实泄压阀（原为死端）；⑥控制台删除实验与后台 LLM 分析线程的 rmtree 竞态改为 409 拒绝。
- 死代码/残留清除：日线回放残留（AT_DECISION_TIME/AT_REPLAY_GRANULARITY 死环境变量与 MainPolicyRunner 双参数，测试还在传 "daily"）；webui legacy 无 hitl/ 与扁平 Step 树兼容分支（宿主已无此类实验且不可再现，缺 control.json 现按未揭示处理——fail-closed 方向）；LEGACY_REPORT_FILES、legacy --*-quarter CLI 别名、RunManifest.load、SandboxPaths 死别名（output 经核实为 getattr 动态消费者、保留并注明）、auction open_mask、DatasetContract.unit_rules、SERVER_MANAGED_KEYS、seriesSpec、部署文档 §4.2 与冻结实现矛盾的过时 payload 示例。
- 单一事实源收敛：~15 个 selected_* 数据集校验器 → 1 个通用 select_datasets；event/board available_at if-阶梯 → 规格上的声明式 AvailabilityRule（穷举等价脚本证明全部 37 规格 PIT 戳字节不变）；ExperimentConfig 三处手写构造 → assembly 单一 builder（新增完整性回归：新字段无法只到达单一入口）；epoch→meta→fold 因果序三处循环 → iter_development_sessions 单生成器（P4 全部因果序测试不变）；Agent/Meta 可见性投影 → pipelines/agent_views.py（迁移经机器验证输出全同）；回测 5 段异常阶梯 → 单一 classify-once 归一路径；llm_observation、工具错误观察、parquet footer 剖析、snapshot manifest writer、runtime_env 记录、StepTree 原子写、cron stable_hash（序列化与 common 逐字节一致故合并保 hash）各收敛为单实现；驱动器/Broker verb→op 映射新增相等性测试锁定（驱动器 stdlib-only 不能 import Broker）。
- 明确不做并记录：backtest._execute 大方法拆分（阶梯已收敛，整体拆分延后）；eager 分钟路径删除（等价性测试以其为参照实现）；NL no_evidence_skips 保持按逻辑调用计数（测试钉住的既定语义，已注释澄清）；SSRF DNS 重绑定 TOCTOU 记为 meta-only 已知限制。
- Validation：全量 tests/unit 950 passed + 45 subtests（基线 938+45；Docker E2E 实跑容器）；PROMPTS.md 双次导出字节一致并新增 round-trip 回归测试守护；每波 git diff --check clean。主工作树全程保持 fix/share-float-incremental-archive 不动（活跃 console、全日 cron、lcd-test1 worker 零扰动），重构全部在 .worktrees/refactor 内完成。与 fix 分支的预期合并冲突面：tushare download/audit/common、artifacts/sandbox/backtest/finish_fold、test_data_sources_tushare、LOGBOOK。
